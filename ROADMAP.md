...
`STEP020A_DURABLE_BACKGROUND_TASK_LEDGER_AND_RUNTIME_LIFECYCLE_FOUNDATION` (`0.20.0-step020a`, schema 18, Windows 40/40), SHA-256 `67ac1fa4a5067ff3070f0a990bfdfd262a6d956961ebd221432cdacf567c9a7f`.

## Current candidate

`STEP020B_DURABLE_TASK_FLOW_REGISTRY_AND_CONTROLLER_LIFECYCLE_FOUNDATION` (`0.20.1-step020b`, schema 19).

Goal: durable controller-owned Flow identity, revisioned lifecycle, waiting/block state, one-Flow-many-Task links, child cancellation cascade and Host restart restoration. Windows harness: `STEP020B_H1_TASK_FLOW_PROTOCOL_RESTART_REVISION_AND_CANCELLATION`.

## Deferred candidates

- controller-driven Task admission/execution based on proven scenarios;
- completion delivery and notification routing;
- Flow audit repair, stuck-cancel/LOST reconciliation and retention;
- distributed ownership and workers;
- external-model, Browser LIVE and real Connector acceptance.

No deferred item is committed merely because OpenClaw implements it.

## Retained integration rule

Mattermost and Connector work remains speculative and deferred until an executable real system exists, together with a concrete real adapter contract and real API/event environment. Plugin runtime and Plugin marketplace breadth are also deferred.
