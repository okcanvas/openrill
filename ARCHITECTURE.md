...
---
# ARCHITECTURE — OpenRill

## 1. 논리 구조

```text
┌─────────────────────────────────────────────────────────────┐
│ apps/agent-web        apps/agent-cli       apps/desktop(*) │
└───────────────────────┬─────────────────────────────────────┘
                        │ OpenRill Local Protocol
┌───────────────────────▼─────────────────────────────────────┐
│ services/agent-host                                        │
│ Bootstrap · Lock · HTTP/WS · Auth · Readiness · Shutdown   │
├─────────────────────────────────────────────────────────────┤
│ Conversation Service │ Agent Run Service │ Automation      │
├─────────────────────────────────────────────────────────────┤
│ Agent Kernel │ Tool Runtime │ Approval │ Skill Runtime      │
├─────────────────────────────────────────────────────────────┤
│ Workspace │ Process │ Browser │ Sandbox │ Connector Adapter │
├─────────────────────────────────────────────────────────────┤
│ SQLite State │ Config Journal │ Artifact Store │ Log Store  │
└─────────────────────────────────────────────────────────────┘

(*) Desktop shell은 Post-MVP이다. 최초 UI는 Host가 제공하는 브라우저 UI다.
```

## 2. 프로세스 모델

초기 구현은 하나의 Host 프로세스와 격리 가능한 자식 프로세스로 제한한다.

- `agent-host`: 포트·락·DB·WebSocket·scheduler를 소유한다.
- command child: Shell 실행마다 명시적 process record와 cancellation을 갖는다.
- browser child: Playwright browser lifecycle을 별도 manager가 소유한다.
- sandbox backend: host 또는 Docker backend로 Tool 실행 위치를 선택한다.
- UI: 정적 asset이며 Host와 동일 origin으로 연결한다.

Agent Kernel을 별도 서버 프로세스로 분리하지 않는다. 장시간 blocking 또는 crash 격리가 실제 측정으로 필요할 때 worker thread/child process ADR을 추가한다.
...
BOOTSTRAP
  ...
  → HOST_LOCK
  → CONFIG_SOURCE_READ
  → CONFIG_VALIDATE_AND_MATERIALIZE
  → STATE_OPEN_AND_MIGRATE
  → HTTP_WS_PREPARE
  → HANDLERS_ATTACH
  → LISTEN
  → BACKGROUND_SERVICES_START
  → READY
```

listen 전 handler 부착, ready 전 background sidecar 확인, 역순 shutdown을 불변조건으로 둔다.

## 4. 모듈 소유권

| 모듈 | 소유 데이터/책임 | 금지 의존성 |
|---|---|---|
| protocol | wire frame/schema/version | DB, UI, model SDK |
| config | source config, materialized snapshot | agent kernel |
| state | SQLite migration/repository, Process backend/confinement evidence | UI component |
| agent-kernel | model turn/tool loop | HTTP server, connector SDK |
| tool-runtime | tool registry/policy/execution | UI route |
| approval | durable prompt/resolution/replay | provider-specific code |
| skills | discovery/validation/snapshot | connector transport |
| automation | schedule/config/runtime state | UI framework |
| connectors | inbound/outbound normalization | agent internals |
| diagnostics | health/doctor/log bundle | mutation business logic |

## 5. 상세 문서

- [docs/architecture/PROCESS_MODEL.md](docs/architecture/PROCESS_MODEL.md)
- [docs/architecture/STARTUP_SHUTDOWN.md](docs/architecture/STARTUP_SHUTDOWN.md)
- [docs/architecture/LOCAL_PROTOCOL.md](docs/architecture/LOCAL_PROTOCOL.md)
- [docs/architecture/AGENT_EXECUTION.md](docs/architecture/AGENT_EXECUTION.md)
- [docs/architecture/FAILURE_RECOVERY.md](docs/architecture/FAILURE_RECOVERY.md)
- [docs/data/SQLITE_MODEL.md](docs/data/SQLITE_MODEL.md)

## 6. Delegated-work foundation

STEP014A adds a durable delegation graph beneath Conversation/Run ownership. `packages/state` owns immutable graph/event/wait rows, `packages/conversations` owns validation and transitions, and `packages/agent-kernel` owns cumulative token/time enforcement. Public Tools, Protocol, scheduler, and UI remain unchanged until later STEP014 waves.

## 7. Execution backend routing

STEP015B routes the actual Agent Host `process.run` path through a provider-neutral execution
backend boundary. Configuration selects `host` or `docker`; Docker unavailability fails closed
unless Host fallback is separately explicit. State schema 15 persists backend kind, handle
identity, sandbox claim, and exact confinement proof on each backend-routed Process record.

Resource ownership order is fixed:

```text
backend selection
  → secret environment resolution
  → backend prepare
  → durable Process/backend binding
  → exec
  → timeout/cancel if needed
  → settlement
  → backend close
```

The Host backend never claims sandboxing. The Docker backend requires a digest-pinned image and
uses read-only root, dropped capabilities, no-new-privileges, bounded memory/PIDs, one workspace
mount, and network none by default. Browser automation is unrelated to this acceptance boundary.
