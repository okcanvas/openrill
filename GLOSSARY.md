...
- **Goal**: Conversation-owned durable objective.
- **Plan**: Revisioned ordered proposal for achieving a Goal. It describes intent; it is not runtime execution.
- **Run**: Authoritative execution lifecycle.
- **Attempt**: One execution try of a Run with provenance.
- **Task**: Durable one-to-one Run-linked activity ledger. It records execution facts and delegates cancellation to the owning Run; it is not a scheduler.
- **Task Flow**: Durable controller-owned orchestration record over multiple Tasks. It owns status, revision, step, wait/block state and cancellation intent; it is not an autonomous Plan executor.
- **Controller**: Component authorized to create and revisionally mutate a Task Flow.
- **Revision CAS**: Expected-revision write boundary that rejects stale controller mutations.
- **Accepted baseline**: `STEP020A_DURABLE_BACKGROUND_TASK_LEDGER_AND_RUNTIME_LIFECYCLE_FOUNDATION`, SHA-256 `67ac1fa4a5067ff3070f0a990bfdfd262a6d956961ebd221432cdacf567c9a7f`.
- **Current candidate**: `STEP020B_DURABLE_TASK_FLOW_REGISTRY_AND_CONTROLLER_LIFECYCLE_FOUNDATION`, version `0.20.1-step020b`, schema 19.
