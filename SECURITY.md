...
OpenRill은 사용자 계정으로 실행되므로 sandbox가 없으면 해당 사용자 권한을 가진다. 따라서 “로컬 실행”을 “안전한 실행”으로 간주하지 않는다.

## 기본값

- HTTP/WS bind: loopback only
- 원격 origin: 거부
- Shell: `PROMPT` 기본
- 파일 쓰기: Workspace root 내부만
- Workspace 외부 읽기: 거부
- Secret: OS keychain 또는 환경 변수 참조
- 자동화의 위험 Tool: foreground와 동일한 policy 적용
- Connector 입력: 신뢰하지 않는 외부 입력으로 분류
- Plugin/Extension: MVP 비활성
...
해결된 승인 id를 재사용한 실행은 거부한다. `allow_once`는 정확히 한 번만 소비된다.

## 상세 문서

- [docs/security/THREAT_MODEL.md](docs/security/THREAT_MODEL.md)
- [docs/security/EXECUTION_POLICY.md](docs/security/EXECUTION_POLICY.md)
