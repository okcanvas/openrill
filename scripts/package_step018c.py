...
def main():
    p=argparse.ArgumentParser(description="Create deterministic OpenRill STEP018C source ZIP"); p.add_argument("--output",type=Path,required=True); a=p.parse_args()
    ...
    digest=hashlib.sha256(out.read_bytes()).hexdigest(); print(f"OPENRILL_STEP018C_PACKAGE_PASS files={len(files)} sha256={digest}")
    return 0
