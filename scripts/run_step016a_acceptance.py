...
REPORT = resolve_acceptance_report(ROOT, ".artifacts/acceptance/STEP016A_ACCEPTANCE_REPORT.txt")
STAGE_LOG_DIR = REPORT.parent / "STEP016A_STAGES"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run OpenRill STEP016A source/package acceptance")
    ...
    check("root-description", "STEP016A" in package.get("description", ""))
    check("acceptance-script", scripts.get("acceptance:step016a") == "python scripts/run_step016a_acceptance.py")
    check("live-acceptance-script", scripts.get("acceptance:step016a:live") == "python scripts/run_step016a_acceptance.py --require-windows-dpapi-live")
    check("live-script", scripts.get("windows-dpapi-live:step016a") == "node scripts/run-step016a-windows-dpapi-live.mjs")
    check("package-script", scripts.get("package:step016a") == "python scripts/package_step016a.py --output ../openrill-step016a-local-setup-doctor-windows-dpapi-secret-foundation-v1.zip")
    ...
    required = [
        ...
        "tests/unit/local-setup-doctor-step016a.test.mjs",
        "tests/unit/validation-governance-step016a.test.mjs",
        "scripts/run-step016a-windows-dpapi-live.mjs",
        "docs/plans/STEP016A_LOCAL_SETUP_DOCTOR_AND_WINDOWS_DPAPI_SECRET_FOUNDATION.md",
    ...
    runner = read_utf8("scripts/run_step016a_acceptance.py")
    plan = read_utf8("docs/plans/STEP016A_LOCAL_SETUP_DOCTOR_AND_WINDOWS_DPAPI_SECRET_FOUNDATION.md")
    ...
    if args.require_windows_dpapi_live:
        if os.name != "nt":
            ...
            stages.append(("windows-dpapi-live", ["node", "scripts/run-step016a-windows-dpapi-live.mjs"], 180))
            live_state = "RUNNING"

    for name, command, timeout in stages:
        ok, output, elapsed = run_utf8(name, command, timeout)
        automated_seconds += elapsed
        if name == "focused-step016a-product":
    ...
    marker = (
        ...
        "os_secret=WINDOWS_DPAPI_SOURCE_ACCEPTED secret_persistence=REFERENCE_ONLY model_network=NOT_RUN "
        f"browser=NOT_RUN connector=DEFERRED_NO_REAL_SYSTEM windows_dpapi_live={live_state} promotion={promotion} automated_run_seconds={automated_seconds:.3f}"
    )
    lines = [marker]
    for name, ok, detail in checks:
        if not ok:
            lines.append(f"OPENRILL_STEP016A_FAILURE check={name}")
            if detail:
                lines.append(detail)
