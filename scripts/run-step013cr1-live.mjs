...
const marker = `openrill-step013cr1-${randomUUID()}`;
const profile = "step013cr1-live";
...
function toolEvents(requestNumber, name, args) {
  const callId = `step013cr1-call-${requestNumber}-${name.replace(/[^a-z]/g, "-")}`;
  return [
...
function finalEvents(requestNumber) {
  return [
    { type: "response.created", response: { id: `response-${requestNumber}` } },
    { type: "response.output_text.delta", delta: "STEP013CR1 autonomous browser run completed" },
    { type: "response.completed", response: { id: `response-${requestNumber}`, usage: { input_tokens: 1, output_tokens: 1, total_tokens: 2 } } },
  ];
}
...
async function startFixtureServer() {
  const server = createServer((request, response) => {
    ...
    if (url.pathname === "/page") {
      ...
      response.end(`<!doctype html><html><head><title>STEP013CR1 fixture</title></head><body><h1>Automation Browser</h1><p>restart recovery</p><script>console.log("step013cr1-console");fetch("/api?token=secret-step013cr1");</script></body></html>`);
      return;
    }
    if (url.pathname === "/api") {
...
function spawnChild(env) {
  const child = spawn(process.execPath, [join(import.meta.dirname, "run-step013cr1-host-child.mjs")], { cwd: join(import.meta.dirname, ".."), env, stdio: ["pipe", "pipe", "pipe"], windowsHide: true });
  let output = "";
...
const root = await mkdtemp(join(tmpdir(), "openrill-step013cr1-live-"));
...
try {
  ...
  const job = definitions.create({
    name: "STEP013CR1 autonomous browser",
    enabled: false,
  ...
  const scheduled = definitions.runNow(job.jobId, "step013cr1-live-request").run;
  ...
  const childEnv = {
    ...process.env,
    ...envRoot,
    OPENRILL_STEP013CR1_PROFILE: profile,
    OPENRILL_STEP013CR1_WORKSPACE: workspacePath,
    OPENRILL_STEP013CR1_MODEL_ENDPOINT: model.endpoint,
    OPENRILL_STEP013CR1_BROWSER_MARKER: marker,
  };
  ...
  if (terminal.status !== "SUCCEEDED") {
    ...
    process.stderr.write(`OPENRILL_STEP013CR1_RECOVERY_DIAGNOSTICS ${JSON.stringify(diagnostics)}\n`);
  }
  ...
  assert.doesNotMatch(durableText, /secret-step013cr1/);
  ...
  console.log(`OPENRILL_STEP013CR1_LIVE_PASS automation=BROWSER_RUN ledger=ACTION_EVIDENCE recovery=RESUME_AND_REOPEN model_requests=${model.requestCount()} process_count=0 chromium_orphan=0`);
} catch (error) {
  primaryError = error;
} finally {
  ...
  if (cleanupFailures.length > 0) {
    ...
    if (primaryError) process.stderr.write(`OPENRILL_STEP013CR1_CLEANUP_AFTER_FAILURE ${detail}\n`);
    else primaryError = new AggregateError(cleanupFailures, `STEP013CR1 live cleanup failed: ${detail}`);
  }
}
