...
STEP="STEP020ER1_WINDOWS_LOCAL_PROTOCOL_RESTART_CONNECT_RETRY_CLOSURE"; VERSION="0.20.6-step020er1"; SCHEMA=22
LIVE_HARNESS="STEP020ER1_H1_WINDOWS_LOCAL_PROTOCOL_RESTART_CONNECT_RETRY_AND_COMPLETION"
...
REPORT=resolve_acceptance_report(ROOT,".artifacts/acceptance/STEP020ER1_ACCEPTANCE_REPORT.txt"); LOGDIR=REPORT.parent/"STEP020ER1_STAGES"
PRODUCT=[
...
GOVERNANCE=[
 ...
 "tests/unit/validation-governance-step020a.test.mjs","tests/unit/validation-governance-step020b.test.mjs","tests/unit/validation-governance-step020br1.test.mjs","tests/unit/validation-governance-step020c.test.mjs","tests/unit/validation-governance-step020d.test.mjs","tests/unit/validation-governance-step020e.test.mjs","tests/unit/validation-governance-step020er1.test.mjs",
]
STAGES=[
 ...
 ("focused-step020er1-product",["node","--test","--test-concurrency=1","--test-reporter=tap",*PRODUCT],500),
...
def main():
    parser=argparse.ArgumentParser(); parser.add_argument("--require-windows-completion-retry-live",action="store_true"); args=parser.parse_args()
    ...
    check("root-version",package.get("version")==VERSION,str(package.get("version"))); check("root-description","STEP020ER1" in package.get("description",""));
    check("acceptance-script",scripts.get("acceptance:step020er1")=="python scripts/run_step020er1_acceptance.py"); check("live-script",scripts.get("acceptance:step020er1:live")=="python scripts/run_step020er1_acceptance.py --require-windows-completion-retry-live"); check("completion-live-script",scripts.get("completion-retry-live:step020er1")=="node scripts/run-step020er1-completion-retry-live.mjs")
    ...
    required=[
      ...
      *PRODUCT,"tests/unit/validation-governance-step020e.test.mjs","docs/research/STEP020E_OPENCLAW_COMPLETION_DELIVERY_AND_CONTROLLER_WAKE_AUDIT.md","docs/plans/STEP020ER1_WINDOWS_LOCAL_PROTOCOL_RESTART_CONNECT_RETRY_CLOSURE.md",
      ...
      "apps/agent-cli/src/local-protocol-client.ts","tests/unit/local-cli-protocol-retry-step020er1.test.mjs","tests/unit/validation-governance-step020er1.test.mjs",
      "reference/validation/STEP020E_WINDOWS_COMPLETION_LIVE_FAILURE.md","reference/validation/STEP020ER1_OR_ISSUE_269.md","reference/validation/STEP020ER1_LOCAL_SOURCE_PACKAGE_ACCEPTANCE.md","docs/plans/STEP020ER1_WINDOWS_LOCAL_PROTOCOL_RESTART_CONNECT_RETRY_CLOSURE.md",
      "scripts/run-step020er1-completion-retry-live.mjs","scripts/package_step020er1.py",
    ]
    ...
    if args.require_windows_completion_retry_live:
        result=run("windows-completion-retry-live",["node","scripts/run-step020er1-completion-retry-live.mjs"],700); seconds+=result.elapsed_seconds
        marker=f"{STEP} checks=21/21 state=PASSED version={VERSION} schema={SCHEMA} correction=BOUNDED_TRANSPORT_ONLY_RETRY timeout=CALLER_DEADLINE security=NON_TRANSPORT_FAIL_FAST delivery=DURABLE_TASK_EVENT semantics=REQUIRED_COMPLETION controller=OWNER_CONVERSATION_WAKE queue=SYSTEM_MESSAGE_WAKE_RUN restart=PENDING_DRAIN_IDENTITY_STABLE scope=CONTROLLER_TOOLS_DURABLE decision=EXPLICIT_TOOL_REQUIRED migration=TERMINAL_CHILD_SAFE_BACKFILL flow=CONTROLLER_OWNED_OUTCOME plan_executor=DEFERRED provider=SCRIPTED_LOCAL live_harness={LIVE_HARNESS}"
        check("windows-completion-retry-live",result.ok and marker in result.output,result.output[-5000:])
    passed=sum(1 for _,ok,_ in checks if ok); total=len(checks); state="PASSED" if passed==total else "FAILED"; live="PASSED" if args.require_windows_completion_retry_live and state=="PASSED" else "PENDING_ENV" if not args.require_windows_completion_retry_live else "FAILED"; promotion="READY" if live=="PASSED" else "WINDOWS_COMPLETION_RETRY_LIVE_PENDING" if live=="PENDING_ENV" else "BLOCKED"
    marker=(f"{STEP} checks={passed}/{total} state={state} version={VERSION} schema={SCHEMA} accepted_product_baseline={BASELINE} accepted_checks={BASELINE_CHECKS} correction=BOUNDED_TRANSPORT_ONLY_RETRY timeout=CALLER_DEADLINE security=NON_TRANSPORT_FAIL_FAST delivery=DURABLE_TASK_EVENT semantics=REQUIRED_COMPLETION controller=OWNER_CONVERSATION_WAKE queue=SYSTEM_MESSAGE_WAKE_RUN restart=PENDING_DRAIN_IDENTITY_STABLE scope=CONTROLLER_TOOLS_DURABLE decision=EXPLICIT_TOOL_REQUIRED migration=TERMINAL_CHILD_SAFE_BACKFILL flow=CONTROLLER_OWNED_OUTCOME plan_executor=DEFERRED openclaw_reference=COMPLETION_DELIVERY_SOURCE_AUDITED external_model=NOT_RUN browser_live=NOT_RUN connector=DEFERRED_NO_REAL_SYSTEM focused_product={counts.get('focused-step020er1-product',0)} affected_regression={counts.get('affected-completion-regression',0)} governance={counts.get('focused-validation-governance',0)} canonical_files={counts.get('canonical-files',0)} canonical_tests={counts.get('canonical-suite',0)} windows_completion_retry_live={live} live_harness={LIVE_HARNESS} promotion={promotion} automated_run_seconds={seconds:.3f}")
    lines=[marker]+[f"OPENRILL_STEP020ER1_FAILURE check={name}\n{detail}" for name,ok,detail in checks if not ok]; write_acceptance_report(REPORT,"\n".join(lines)+"\n"); print(marker); [print(line) for line in lines[1:]]; return 0 if state=="PASSED" else 1
