...
STEP = "STEP014DR3_OPENAI_RESPONSES_STREAM_TOOL_IDENTITY_UNIFICATION_AND_EMPTY_NAME_FAIL_CLOSED"
...
REPORT = resolve_acceptance_report(ROOT, ".artifacts/acceptance/STEP014DR3_ACCEPTANCE_REPORT.txt")
STAGE_LOG_DIR = REPORT.parent / "STEP014DR3_STAGES"
...
STAGES: list[tuple[str, list[str], int]] = [
    ...
    ("external-model-control-ui-live", ["node", "scripts/run-step014dr3-live.mjs"], 300),
...
def main() -> int:
    ...
    check("acceptance-script", scripts.get("acceptance:step014dr3") == "python scripts/run_step014dr3_acceptance.py")
    check("package-script", scripts.get("package:step014dr3") == "python scripts/package_step014dr3.py --output ../openrill-step014dr3-openai-responses-stream-tool-identity-unification-empty-name-fail-closed-v1.zip")
    ...
    required = [
        ...
        "tests/unit/delegation-control-step014d.test.mjs", "tests/unit/delegation-control-boundaries-step014d.test.mjs", "tests/unit/model-openai-responses-step014dr2.test.mjs", "tests/unit/step014dr2-boundaries.test.mjs", "tests/unit/model-openai-responses-step014dr3.test.mjs", "tests/unit/step014dr3-boundaries.test.mjs",
        "scripts/run-step014d-live.mjs", "scripts/run-step014dr1-live.mjs", "scripts/run-step014dr2-live.mjs", "scripts/run-step014dr3-live.mjs", "scripts/step014dr1-live-diagnostics.mjs",
        "scripts/check_source_root_boundary.py", "scripts/run_step014dr1_acceptance.py", "scripts/package_step014dr1.py", "scripts/run_step014dr2_acceptance.py", "scripts/package_step014dr2.py", "scripts/run_step014dr3_acceptance.py", "scripts/package_step014dr3.py",
        "scripts/sh_run_step014d_acceptance.cmd", "scripts/sh_run_step014d_acceptance.sh",
        "scripts/sh_run_step014dr1_acceptance.cmd", "scripts/sh_run_step014dr1_acceptance.sh", "scripts/sh_run_step014dr2_acceptance.cmd", "scripts/sh_run_step014dr2_acceptance.sh", "scripts/sh_run_step014dr3_acceptance.cmd", "scripts/sh_run_step014dr3_acceptance.sh",
        "docs/plans/STEP014DR3_OPENAI_RESPONSES_STREAM_TOOL_IDENTITY_UNIFICATION_AND_EMPTY_NAME_FAIL_CLOSED.md",
        "docs/contracts/DELEGATED_WORK_CONTROL_SURFACE.md", "docs/contracts/MODEL_ADAPTER.md", "docs/adrs/ADR-0033-PROVIDER_SAFE_TOOL_ALIASES_WITH_CANONICAL_RUNTIME_NAMES.md", "docs/adrs/ADR-0034-OPENAI_RESPONSES_ITEM_AND_CALL_ID_UNIFICATION.md", "docs/validation/STEP014DR2_FAILURE_PREVENTION_AUDIT.md", "docs/validation/STEP014DR3_FAILURE_PREVENTION_AUDIT.md",
        ...
        "reference/validation/STEP014DR1_LOCAL_DETERMINISTIC_VALIDATION.md", "reference/validation/STEP014DR2_LOCAL_DETERMINISTIC_VALIDATION.md", "reference/validation/STEP014DR3_LOCAL_DETERMINISTIC_VALIDATION.md",
    ...
    ] + [f"reference/validation/STEP014D_OR_ISSUE_{number}.md" for number in range(146, 157)] + [f"reference/validation/STEP014DR1_OR_ISSUE_{number}.md" for number in range(157, 161)] + [f"reference/validation/STEP014DR2_OR_ISSUE_{number}.md" for number in range(161, 164)]
    ...
    marker = (
        ...
        "external_model=NESTED_PARALLEL chromium=RENDERED privacy=BOUNDED diagnostics=TYPED_AND_PRESERVED source_root=ARCHIVE_FREE provider_tool_names=ALIASED canonical_tools=ROUND_TRIP stream_tool_identity=UNIFIED empty_tool_name=FAIL_CLOSED reporter=TAP chromium_orphan=0"
    )
    lines = [marker]
    for name, ok, detail in checks:
        if not ok:
            lines.append(f"OPENRILL_STEP014DR3_FAILURE check={name}")
            if detail:
                lines.append(detail)
