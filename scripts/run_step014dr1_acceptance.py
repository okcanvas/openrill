...
STEP = "STEP014DR1_SOURCE_ROOT_ARCHIVE_BOUNDARY_AND_EXTERNAL_MODEL_FAILURE_DIAGNOSTICS"
...
REPORT = resolve_acceptance_report(ROOT, ".artifacts/acceptance/STEP014DR1_ACCEPTANCE_REPORT.txt")
STAGE_LOG_DIR = REPORT.parent / "STEP014DR1_STAGES"
...
STAGES: list[tuple[str, list[str], int]] = [
    ...
    ("external-model-control-ui-live", ["node", "scripts/run-step014dr1-live.mjs"], 300),
...
def main() -> int:
    ...
    check("acceptance-script", scripts.get("acceptance:step014dr1") == "python scripts/run_step014dr1_acceptance.py")
    check("package-script", scripts.get("package:step014dr1") == "python scripts/package_step014dr1.py --output ../openrill-step014dr1-source-root-archive-boundary-external-model-failure-diagnostics-v1.zip")
    ...
    required = [
        ...
        "scripts/run-step014d-live.mjs", "scripts/run-step014dr1-live.mjs", "scripts/step014dr1-live-diagnostics.mjs",
        "scripts/check_source_root_boundary.py", "scripts/run_step014dr1_acceptance.py", "scripts/package_step014dr1.py",
        "scripts/sh_run_step014d_acceptance.cmd", "scripts/sh_run_step014d_acceptance.sh",
        "scripts/sh_run_step014dr1_acceptance.cmd", "scripts/sh_run_step014dr1_acceptance.sh",
        "docs/plans/STEP014DR1_SOURCE_ROOT_ARCHIVE_BOUNDARY_AND_EXTERNAL_MODEL_FAILURE_DIAGNOSTICS.md",
        ...
        "reference/validation/STEP014DR1_LOCAL_DETERMINISTIC_VALIDATION.md",
        "config/current-accepted-baseline.json",
    ] + [f"reference/validation/STEP014D_OR_ISSUE_{number}.md" for number in range(146, 157)] + [f"reference/validation/STEP014DR1_OR_ISSUE_{number}.md" for number in range(157, 161)]
    ...
    marker = (
        ...
        "external_model=NESTED_PARALLEL chromium=RENDERED privacy=BOUNDED diagnostics=TYPED_AND_PRESERVED source_root=ARCHIVE_FREE reporter=TAP chromium_orphan=0"
    )
    lines = [marker]
    for name, ok, detail in checks:
        if not ok:
            lines.append(f"OPENRILL_STEP014DR1_FAILURE check={name}")
            if detail:
                lines.append(detail)
