...
import { loadStep020er3LiveMarkerContract, renderStep020er3LiveMarker } from "./step020er3-live-marker.mjs";

const contract = await loadStep020er3LiveMarkerContract();
if(process.platform!=="win32") throw new Error("OPENRILL_STEP020ER3_WINDOWS_REQUIRED");
...
check("focused-tests",tap("tests")===20,String(tap("tests")));
check("focused-pass",tap("pass")===20,String(tap("pass")));
...
check("marker-contract-identity",contract.step==="STEP020ER3_WINDOWS_PYTHON_LIVE_MARKER_VALIDATOR_ENTRYPOINT_CLOSURE"&&contract.expectedChecks==="26/26"&&contract.liveHarness==="STEP020ER3_H1_WINDOWS_PYTHON_VALIDATOR_ENTRYPOINT_AND_COMPLETION");
check("marker-contract-required-fields",contract.fields.queue==="SYSTEM_MESSAGE_WAKE_RUN"&&contract.fields.migration==="TERMINAL_CHILD_SAFE_BACKFILL"&&contract.fields.validation==="NO_PYTHONPATH_ASSUMPTION");
check("python-absolute-entrypoint",focused.output.includes("absolute Python file entrypoint"));
check("python-external-cwd",focused.output.includes("external cwd without PYTHONPATH"));
check("python-shadow-package-isolated",focused.output.includes("ignores a shadow scripts package"));
...
console.log(renderStep020er3LiveMarker(contract,{passed,total:checks.length,state}));
for(const item of checks.filter(i=>!i.passed)) console.error(`OPENRILL_STEP020ER3_LIVE_FAILURE check=${item.name} detail=${item.detail}`);
