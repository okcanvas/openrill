...
STEP = "STEP013AR1_WORKSPACE_LOCK_IMPORTER_ALIGNMENT"
...
REPORT = resolve_acceptance_report(ROOT, ".artifacts/acceptance/STEP013AR1_ACCEPTANCE_REPORT.txt")
PACKAGED_REPORT = ROOT / "reference/validation/STEP013AR1_ACCEPTANCE_REPORT.txt"
...
def main() -> int:
    ...
    check("step013ar1-script", scripts.get("acceptance:step013ar1") == "python scripts/run_step013ar1_acceptance.py")
    check("step013ar1-package-script", scripts.get("package:step013ar1") == "python scripts/package_step013ar1.py --output ../openrill-step013ar1-workspace-lock-importer-alignment-v1.zip")
    ...
    required = [
        ...
        "scripts/run_step013ar1_acceptance.py",
        "scripts/package_step013ar1.py",
        "scripts/sh_run_step013ar1_acceptance.cmd",
        "scripts/sh_run_step013ar1_acceptance.sh",
        "tests/unit/workspace-lock-alignment-step013ar1.test.mjs",
        "docs/plans/STEP013AR1_WORKSPACE_LOCK_IMPORTER_ALIGNMENT.md",
        ...
        "reference/validation/STEP013AR1_ACCEPTANCE_REPORT.txt",
        "scripts/sh_run_step013ar1_acceptance.cmd",
        "scripts/sh_run_step013ar1_acceptance.sh",
    ...
    plan = read_utf8(ROOT / "docs/plans/STEP013AR1_WORKSPACE_LOCK_IMPORTER_ALIGNMENT.md")
    for heading in (
    ...
    check("failure-prevention-audit", all(token in audit for token in ("single-flight", "final URL", "OR-ISSUE-081", "OR-ISSUE-083", "OR-ISSUE-085", "STEP013B")))
    ...
    cmd_bytes = (ROOT / "scripts/sh_run_step013ar1_acceptance.cmd").read_bytes()
    check("windows-launcher-crlf", b"\r\n" in cmd_bytes and b"\n" not in cmd_bytes.replace(b"\r\n", b""))
    check("windows-launcher-root-relative", b"%~dp0.." in cmd_bytes)
    check("posix-launcher", 'cd "$SCRIPT_DIR/.."' in read_utf8(ROOT / "scripts/sh_run_step013ar1_acceptance.sh"))
    ...
    check("workspace-module-links-current-root", links_ok and "OPENRILL_WORKSPACE_MODULE_LINKS_PASS" in links_output and "root_owned=true" in links_output, links_output.strip())
    ...
    marker = (
        ...
        "retained_feature=STEP013A lock_importers=EXACT pnpm_run=NO_IMPLICIT_INSTALL "
