...
CONTRACT_PATH = ROOT / "config/step023a-live-marker-contract.json"
def load_contract(path: Path = CONTRACT_PATH) -> dict:
    value = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(value, dict) or not isinstance(value.get("fields"), dict): raise ValueError("OPENRILL_STEP023A_LIVE_MARKER_CONTRACT_INVALID")
    return value
...
def validate_live_output(output: str, contract: dict | None=None) -> tuple[bool,str]:
    ...
    return ok,f"step={step} missing={missing} extra={extra} changed={[(k,expected[k],actual[k]) for k in changed]}"
