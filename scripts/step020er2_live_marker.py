...
CONTRACT_PATH = ROOT / "config/step020er2-live-marker-contract.json"


def load_contract() -> dict:
    contract = json.loads(CONTRACT_PATH.read_text(encoding="utf-8"))
    if not isinstance(contract, dict) or not isinstance(contract.get("fields"), dict):
        raise ValueError("OPENRILL_STEP020ER2_LIVE_MARKER_CONTRACT_INVALID")
    return contract
...
def validate_live_output(output: str, contract: dict | None = None) -> tuple[bool, str]:
    ...
    detail = f"step={step} missing={missing} extra={extra} changed={[(key, expected[key], actual[key]) for key in changed]}"
    return ok, detail


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate the STEP020ER2 Windows LIVE marker from stdin")
