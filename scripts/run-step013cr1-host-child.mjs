...
const profile = required("OPENRILL_STEP013CR1_PROFILE");
const workspacePath = required("OPENRILL_STEP013CR1_WORKSPACE");
const modelEndpoint = required("OPENRILL_STEP013CR1_MODEL_ENDPOINT");
const marker = required("OPENRILL_STEP013CR1_BROWSER_MARKER");
const env = {
  ...process.env,
  OPENRILL_DATA_ROOT: required("OPENRILL_DATA_ROOT"),
  OPENRILL_CONFIG_ROOT: required("OPENRILL_CONFIG_ROOT"),
  OPENRILL_STEP013CR1_API_KEY: "fixture-key",
};
const launchArgs = [
  "--disable-background-networking",
  "--disable-component-update",
  "--no-first-run",
  `--openrill-step013cr1-marker=${marker}`,
...
const config = {
  ...
  modelProviders: {
    default: {
      ...
      apiKey: { kind: "env", key: "OPENRILL_STEP013CR1_API_KEY" },
      model: "step013cr1-fixture",
      maxOutputTokens: 256,
      maxRetries: 0,
    },
  },
...
await host.ready;
console.log(`OPENRILL_STEP013CR1_CHILD_READY pid=${process.pid} port=${host.port} browser_processes=${driver.activeProcessCount}`);
...
const close = async (reason) => {
  ...
  try {
    ...
    await host.closed;
    console.log(`OPENRILL_STEP013CR1_CHILD_CLOSED pid=${process.pid} browser_processes=${driver.activeProcessCount}`);
    process.exitCode = 0;
  } catch (error) {
    console.error(error);
    process.exitCode = 1;
