...
STEP = "STEP014DR6_EXTERNAL_MODEL_ACCEPTANCE_DETERMINISM_AND_NESTED_UI_EVIDENCE_SEPARATION"
...
REPORT = resolve_acceptance_report(ROOT, ".artifacts/acceptance/STEP014DR6_ACCEPTANCE_REPORT.txt")
STAGE_LOG_DIR = REPORT.parent / "STEP014DR6_STAGES"
...
STAGES: list[tuple[str, list[str], int]] = [
    ...
    ("external-model-parallel-live", ["node", "scripts/run-step014dr6-external-model-live.mjs"], 300),
    ("deterministic-nested-control-ui-live", ["node", "scripts/run-step014dr6-deterministic-nested-ui-live.mjs"], 180),
...
def main() -> int:
    ...
    check("acceptance-script", scripts.get("acceptance:step014dr6") == "python scripts/run_step014dr6_acceptance.py")
    check("package-script", scripts.get("package:step014dr6") == "python scripts/package_step014dr6.py --output ../openrill-step014dr6-external-model-acceptance-determinism-nested-ui-evidence-separation-v1.zip")
    ...
    required = [
        ...
        "docs/plans/STEP014DR5_CONTROL_UI_ENTRYPOINT_DISCOVERY_AND_STATIC_ASSET_ROUTE_ALIGNMENT.md", "docs/plans/STEP014DR6_EXTERNAL_MODEL_ACCEPTANCE_DETERMINISM_AND_NESTED_UI_EVIDENCE_SEPARATION.md",
    ...
    external_live = read_utf8("scripts/run-step014dr6-external-model-live.mjs")
    nested_ui_live = read_utf8("scripts/run-step014dr6-deterministic-nested-ui-live.mjs")
    ...
    for name, command, timeout in STAGES:
        ...
        if name == "external-model-parallel-live":
            ok = ok and "STEP014DR6_EXTERNAL_MODEL_PARALLEL_PASS" in output
        if name == "deterministic-nested-control-ui-live":
            ok = ok and "STEP014DR6_DETERMINISTIC_NESTED_UI_PASS" in output and "chromium_orphan=0" in output
