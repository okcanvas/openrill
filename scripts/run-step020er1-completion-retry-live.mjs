...
const SCHEMA=22;
const LIVE_HARNESS="STEP020ER1_H1_WINDOWS_LOCAL_PROTOCOL_RESTART_CONNECT_RETRY_AND_COMPLETION";
if(process.platform!=="win32") throw new Error("OPENRILL_STEP020ER1_WINDOWS_REQUIRED");
...
check("focused-tests",tap("tests")===13,String(tap("tests")));
check("focused-pass",tap("pass")===13,String(tap("pass")));
...
console.log(`${STEP} checks=${passed}/${checks.length} state=${state} version=${VERSION} schema=${SCHEMA} correction=BOUNDED_TRANSPORT_ONLY_RETRY timeout=CALLER_DEADLINE security=NON_TRANSPORT_FAIL_FAST delivery=DURABLE_TASK_EVENT semantics=REQUIRED_COMPLETION controller=OWNER_CONVERSATION_WAKE restart=PENDING_DRAIN_IDENTITY_STABLE scope=CONTROLLER_TOOLS_DURABLE decision=EXPLICIT_TOOL_REQUIRED flow=CONTROLLER_OWNED_OUTCOME plan_executor=DEFERRED provider=SCRIPTED_LOCAL live_harness=${LIVE_HARNESS}`);
for(const item of checks.filter(i=>!i.passed)) console.error(`OPENRILL_STEP020ER1_LIVE_FAILURE check=${item.name} detail=${item.detail}`);
