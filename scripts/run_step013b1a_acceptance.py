...
REPORT = resolve_acceptance_report(ROOT, ".artifacts/acceptance/STEP013B1A_ACCEPTANCE_REPORT.txt")
...
def main() -> int:
    ...
    check("acceptance-script", scripts.get("acceptance:step013b1a") == "python scripts/run_step013b1a_acceptance.py")
    check("package-script", scripts.get("package:step013b1a") == "python scripts/package_step013b1a.py --output ../openrill-step013b1a-windows-deterministic-focused-test-reporter-alignment-v1.zip")
    ...
    required = [
        ...
        "scripts/run_step013b1a_acceptance.py",
        "scripts/package_step013b1a.py",
        "scripts/sh_run_step013b1a_acceptance.cmd",
        "scripts/sh_run_step013b1a_acceptance.sh",
        "tests/unit/focused-test-reporter-step013b1a.test.mjs",
        "docs/plans/STEP013B1A_WINDOWS_DETERMINISTIC_FOCUSED_TEST_REPORTER_ALIGNMENT.md",
        "docs/validation/STEP013B1A_FOCUSED_TEST_REPORTER_FAILURE_PREVENTION_AUDIT.md",
    ...
    current_runner = read_utf8(ROOT / "scripts/run_step013b1a_acceptance.py")
    predecessor_runner = read_utf8(ROOT / "scripts/run_step013b1_acceptance.py")
    focused_files = (
    ...
    for filename in focused_files:
        pattern = rf'\["node", "--test", "--test-concurrency=1", "--test-reporter=tap", "tests/unit/{re.escape(filename)}"\]'
        check(f"tap-reporter-current:{filename}", re.search(pattern, current_runner) is not None)
        check(f"tap-reporter-predecessor:{filename}", re.search(pattern, predecessor_runner) is not None)
    check("tap-predicate-owned", "def tap_pass" in current_runner and "# tests" in current_runner and "# pass" in current_runner)

    stages = [
        ("source-version-alignment", ["python", "scripts/verify_source_version_alignment.py"], lambda output: "OPENRILL_SOURCE_VERSION_ALIGNMENT_PASS version=0.13.6-step013b1a manifests=27 sources=26 host_literals=3" in output),
    ...
    marker = (
        ...
        "retained_feature=STEP013B1 adapter=PLAYWRIGHT_CORE tools=READ_ONLY_6 refs=DOCUMENT_GENERATION_SCOPED "
        "stale_ref=BROWSER_STALE_REF reporter=TAP process_count=0 chromium_orphan=0"
    )
    lines.append(marker)
