...
STEP = "STEP014DR2_OPENAI_RESPONSES_FUNCTION_NAME_ALIAS_AND_CANONICAL_TOOL_ROUND_TRIP"
...
REPORT = resolve_acceptance_report(ROOT, ".artifacts/acceptance/STEP014DR2_ACCEPTANCE_REPORT.txt")
STAGE_LOG_DIR = REPORT.parent / "STEP014DR2_STAGES"
...
STAGES: list[tuple[str, list[str], int]] = [
    ...
    ("external-model-control-ui-live", ["node", "scripts/run-step014dr2-live.mjs"], 300),
...
def main() -> int:
    ...
    check("acceptance-script", scripts.get("acceptance:step014dr2") == "python scripts/run_step014dr2_acceptance.py")
    check("package-script", scripts.get("package:step014dr2") == "python scripts/package_step014dr2.py --output ../openrill-step014dr2-openai-responses-function-name-alias-canonical-tool-round-trip-v1.zip")
    ...
    required = [
        ...
        "tests/unit/delegation-control-step014d.test.mjs", "tests/unit/delegation-control-boundaries-step014d.test.mjs", "tests/unit/model-openai-responses-step014dr2.test.mjs", "tests/unit/step014dr2-boundaries.test.mjs",
        "scripts/run-step014d-live.mjs", "scripts/run-step014dr1-live.mjs", "scripts/run-step014dr2-live.mjs", "scripts/step014dr1-live-diagnostics.mjs",
        "scripts/check_source_root_boundary.py", "scripts/run_step014dr1_acceptance.py", "scripts/package_step014dr1.py", "scripts/run_step014dr2_acceptance.py", "scripts/package_step014dr2.py",
        "scripts/sh_run_step014d_acceptance.cmd", "scripts/sh_run_step014d_acceptance.sh",
        "scripts/sh_run_step014dr1_acceptance.cmd", "scripts/sh_run_step014dr1_acceptance.sh", "scripts/sh_run_step014dr2_acceptance.cmd", "scripts/sh_run_step014dr2_acceptance.sh",
        "docs/plans/STEP014DR2_OPENAI_RESPONSES_FUNCTION_NAME_ALIAS_AND_CANONICAL_TOOL_ROUND_TRIP.md",
        "docs/contracts/DELEGATED_WORK_CONTROL_SURFACE.md", "docs/contracts/MODEL_ADAPTER.md", "docs/adrs/ADR-0033-PROVIDER_SAFE_TOOL_ALIASES_WITH_CANONICAL_RUNTIME_NAMES.md", "docs/validation/STEP014DR2_FAILURE_PREVENTION_AUDIT.md",
        ...
        "reference/validation/STEP014DR1_LOCAL_DETERMINISTIC_VALIDATION.md", "reference/validation/STEP014DR2_LOCAL_DETERMINISTIC_VALIDATION.md",
    ...
    marker = (
        ...
        "external_model=NESTED_PARALLEL chromium=RENDERED privacy=BOUNDED diagnostics=TYPED_AND_PRESERVED source_root=ARCHIVE_FREE provider_tool_names=ALIASED canonical_tools=ROUND_TRIP reporter=TAP chromium_orphan=0"
    )
    lines = [marker]
    for name, ok, detail in checks:
        if not ok:
            lines.append(f"OPENRILL_STEP014DR2_FAILURE check={name}")
            if detail:
                lines.append(detail)
