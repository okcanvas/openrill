...
from step021b_live_marker import load_contract,validate_live_output
ROOT=Path(__file__).resolve().parents[1]
STEP='STEP021B_DURABLE_PLAN_REVISION_RETRY_AND_BLOCKER_RESOLUTION_CLOSURE';VERSION='0.21.1-step021b';SCHEMA=24
CONTRACT=load_contract();LIVE_HARNESS=CONTRACT['liveHarness']
BASELINE='STEP021A_DURABLE_GOAL_PLAN_TO_TASK_FLOW_EXECUTOR_FOUNDATION';BASELINE_CHECKS='58/58';BASELINE_SHA='6193888a454807a65603616fcef146b150e83b18ebc0060e7a577cbd425821fc'
REPORT=resolve_acceptance_report(ROOT,'.artifacts/acceptance/STEP021B_ACCEPTANCE_REPORT.txt');LOGDIR=REPORT.parent/'STEP021B_STAGES'
PRODUCT=['tests/unit/goal-plan-executor-step021a.test.mjs','tests/unit/goal-plan-executor-protocol-step021a.test.mjs','tests/unit/goal-plan-executor-host-step021a.test.mjs','tests/unit/goal-plan-revision-retry-step021b.test.mjs','tests/unit/goal-plan-revision-migration-step021b.test.mjs','tests/unit/goal-plan-revision-retry-protocol-step021b.test.mjs','tests/unit/goal-plan-revision-host-step021b.test.mjs']
AFFECTED=[
...
GOVERNANCE=[
 ...
 'tests/unit/validation-governance-step020a.test.mjs','tests/unit/validation-governance-step020b.test.mjs','tests/unit/validation-governance-step020br1.test.mjs','tests/unit/validation-governance-step020c.test.mjs','tests/unit/validation-governance-step020d.test.mjs','tests/unit/validation-governance-step020e.test.mjs','tests/unit/validation-governance-step020er1.test.mjs','tests/unit/validation-governance-step020er2.test.mjs','tests/unit/validation-governance-step020er3.test.mjs','tests/unit/validation-governance-step021a.test.mjs','tests/unit/validation-governance-step021b.test.mjs']
STAGES=[
 ...
 ('workspace-build',['node','scripts/workspace-runner.mjs','build'],300),('focused-step021b-product',['node','--test','--test-concurrency=1','--test-reporter=tap',*PRODUCT],600),('affected-plan-revision-regression',['node','--test','--test-concurrency=1','--test-reporter=tap',*AFFECTED],1000),('focused-validation-governance',['node','--test','--test-concurrency=1','--test-reporter=tap',*GOVERNANCE],1100),('canonical-suite',['node','scripts/run-canonical-unit-batches.mjs'],1700),('architecture',['python','scripts/check_architecture.py'],120),('exports',['node','scripts/check-exports.mjs'],180),('package-manifest-final',['python','scripts/verify_package_manifest.py'],120)]
def clean():
...
def main():
 parser=argparse.ArgumentParser();parser.add_argument('--require-windows-goal-plan-revision-live',action='store_true');args=parser.parse_args();print('OPENRILL_ACCEPTANCE_STAGE_START name=cleanup timeout_seconds=internal',flush=True);clean();print('OPENRILL_ACCEPTANCE_STAGE_END name=cleanup state=PASS returncode=0 elapsed_seconds=internal',flush=True)
 ...
 check('root-version',pkg.get('version')==VERSION,str(pkg.get('version')));check('root-description','STEP021B' in pkg.get('description',''));check('acceptance-script',scripts.get('acceptance:step021b')=='python scripts/run_step021b_acceptance.py');check('live-script',scripts.get('acceptance:step021b:live')=='python scripts/run_step021b_acceptance.py --require-windows-goal-plan-revision-live');check('goal-live-script',scripts.get('plan-revision-live:step021b')=='node scripts/run-step021b-plan-revision-live.mjs')
 check('baseline-step',baseline.get('step')==BASELINE);check('baseline-checks',baseline.get('checks')==BASELINE_CHECKS);check('baseline-sha',baseline.get('zipSha256')==BASELINE_SHA);check('contract-step',CONTRACT.get('step')==STEP);check('contract-version',CONTRACT.get('version')==VERSION);check('contract-schema',CONTRACT.get('schema')==SCHEMA);check('contract-checks',CONTRACT.get('expectedChecks')=='24/24')
 required=['packages/state/migrations/024_goal_plan_revision_retry_blocker.sql','packages/goal-executor/src/service.ts','packages/state/src/goal-repository.ts','packages/protocol/src/goal-execution-operations.ts','tests/unit/goal-plan-revision-retry-step021b.test.mjs','tests/unit/goal-plan-revision-migration-step021b.test.mjs','tests/unit/goal-plan-revision-retry-protocol-step021b.test.mjs','tests/unit/goal-plan-revision-host-step021b.test.mjs','tests/unit/validation-governance-step021b.test.mjs','scripts/run_step021b_acceptance.py','scripts/run-step021b-plan-revision-live.mjs','scripts/package_step021b.py','config/step021b-live-marker-contract.json','reference/validation/STEP021B_OPENCLAW_SOURCE_AUDIT.md','reference/validation/STEP021B_LOCAL_SOURCE_PACKAGE_ACCEPTANCE.md','reference/validation/STEP021A_WINDOWS_GOAL_PLAN_EXECUTOR_LIVE_ACCEPTANCE.md','docs/plans/STEP021B_DURABLE_PLAN_REVISION_RETRY_AND_BLOCKER_RESOLUTION_CLOSURE.md',*[f'reference/validation/STEP021B_OR_ISSUE_{i}.md' for i in range(291,303)]]
 ...
 if args.require_windows_goal_plan_revision_live:
  result=run('windows-goal-plan-revision-live',['node','scripts/run-step021b-plan-revision-live.mjs'],900);seconds+=result.elapsed_seconds;marker_ok,detail=validate_live_output(result.output,CONTRACT);check('windows-goal-plan-revision-live',result.ok and marker_ok,f'result_ok={result.ok} {detail}\n{result.output[-5000:]}')
 passed=sum(1 for _,ok,_ in checks if ok);total=len(checks);state='PASSED' if passed==total else 'FAILED';live='PASSED' if args.require_windows_goal_plan_revision_live and state=='PASSED' else 'PENDING_ENV' if not args.require_windows_goal_plan_revision_live else 'FAILED';promotion='READY' if live=='PASSED' else 'WINDOWS_GOAL_PLAN_REVISION_LIVE_PENDING' if live=='PENDING_ENV' else 'BLOCKED'
 marker=(f'{STEP} checks={passed}/{total} state={state} version={VERSION} schema={SCHEMA} accepted_product_baseline={BASELINE} accepted_checks={BASELINE_CHECKS} revision=IMMUTABLE_EXECUTION_SNAPSHOT adoption=EXPLICIT_STABLE_STEP retry=MANUAL_BOUNDED_NEW_ATTEMPT blocker=DURABLE_RESOLUTION_LEDGER decision=REVISION_SNAPSHOT_STALE_REJECTED restart=PINNED_REVISION_IDENTITY_STABLE executor=SINGLE_ACTIVE_STEP parallel=DEFERRED openclaw_reference=BOUND_CONTROLLER_SOURCE_REAUDITED external_model=NOT_RUN browser_live=NOT_RUN connector=DEFERRED_NO_REAL_SYSTEM focused_product={counts.get("focused-step021b-product",0)} affected_regression={counts.get("affected-plan-revision-regression",0)} governance={counts.get("focused-validation-governance",0)} canonical_files={counts.get("canonical-files",0)} canonical_tests={counts.get("canonical-suite",0)} windows_goal_plan_revision_live={live} live_harness={LIVE_HARNESS} promotion={promotion} automated_run_seconds={seconds:.3f}')
 lines=[marker]+[f'OPENRILL_STEP021B_FAILURE check={name}\n{detail}' for name,ok,detail in checks if not ok];write_acceptance_report(REPORT,'\n'.join(lines)+'\n');print(marker);[print(x) for x in lines[1:]];return 0 if state=='PASSED' else 1
