...
from step020er2_live_marker import load_contract, validate_live_output
ROOT=Path(__file__).resolve().parents[1]
STEP="STEP020ER2_WINDOWS_COMPLETION_LIVE_MARKER_CONTRACT_ALIGNMENT"; VERSION="0.20.7-step020er2"; SCHEMA=22
...
REPORT=resolve_acceptance_report(ROOT,".artifacts/acceptance/STEP020ER2_ACCEPTANCE_REPORT.txt"); LOGDIR=REPORT.parent/"STEP020ER2_STAGES"
PRODUCT=[
...
GOVERNANCE=[
 ...
 "tests/unit/validation-governance-step020a.test.mjs","tests/unit/validation-governance-step020b.test.mjs","tests/unit/validation-governance-step020br1.test.mjs","tests/unit/validation-governance-step020c.test.mjs","tests/unit/validation-governance-step020d.test.mjs","tests/unit/validation-governance-step020e.test.mjs","tests/unit/validation-governance-step020er1.test.mjs","tests/unit/validation-governance-step020er2.test.mjs",
]
STAGES=[
 ...
 ("focused-step020er2-product",["node","--test","--test-concurrency=1","--test-reporter=tap",*PRODUCT],500),
...
def main():
    parser=argparse.ArgumentParser(); parser.add_argument("--require-windows-completion-marker-live",action="store_true"); args=parser.parse_args()
    ...
    check("root-version",package.get("version")==VERSION,str(package.get("version"))); check("root-description","STEP020ER2" in package.get("description",""))
    check("acceptance-script",scripts.get("acceptance:step020er2")=="python scripts/run_step020er2_acceptance.py"); check("live-script",scripts.get("acceptance:step020er2:live")=="python scripts/run_step020er2_acceptance.py --require-windows-completion-marker-live"); check("completion-live-script",scripts.get("completion-marker-live:step020er2")=="node scripts/run-step020er2-completion-marker-live.mjs")
    ...
    check("contract-step",CONTRACT.get("step")==STEP); check("contract-version",CONTRACT.get("version")==VERSION); check("contract-schema",CONTRACT.get("schema")==SCHEMA); check("contract-checks",CONTRACT.get("expectedChecks")=="23/23"); check("contract-queue",CONTRACT.get("fields",{}).get("queue")=="SYSTEM_MESSAGE_WAKE_RUN"); check("contract-migration",CONTRACT.get("fields",{}).get("migration")=="TERMINAL_CHILD_SAFE_BACKFILL")
    required=[
      "config/step020er2-live-marker-contract.json","scripts/step020er2-live-marker.mjs","scripts/step020er2_live_marker.py","scripts/run-step020er2-completion-marker-live.mjs","scripts/run_step020er2_acceptance.py","scripts/package_step020er2.py",
      "tests/unit/live-marker-contract-step020er2.test.mjs","tests/unit/validation-governance-step020er2.test.mjs",
      "reference/validation/STEP020ER1_WINDOWS_LIVE_MARKER_CONTRACT_FAILURE.md","reference/validation/STEP020ER2_OR_ISSUE_270.md","reference/validation/STEP020ER2_OR_ISSUE_271.md","reference/validation/STEP020ER2_LOCAL_SOURCE_PACKAGE_ACCEPTANCE.md","docs/plans/STEP020ER2_WINDOWS_COMPLETION_LIVE_MARKER_CONTRACT_ALIGNMENT.md",
    ...
    if args.require_windows_completion_marker_live:
        result=run("windows-completion-marker-live",["node","scripts/run-step020er2-completion-marker-live.mjs"],700); seconds+=result.elapsed_seconds
        marker_ok,marker_detail=validate_live_output(result.output,CONTRACT)
        check("windows-completion-marker-live",result.ok and marker_ok,f"result_ok={result.ok} {marker_detail}\n{result.output[-5000:]}")
    passed=sum(1 for _,ok,_ in checks if ok); total=len(checks); state="PASSED" if passed==total else "FAILED"; live="PASSED" if args.require_windows_completion_marker_live and state=="PASSED" else "PENDING_ENV" if not args.require_windows_completion_marker_live else "FAILED"; promotion="READY" if live=="PASSED" else "WINDOWS_COMPLETION_MARKER_LIVE_PENDING" if live=="PENDING_ENV" else "BLOCKED"
    marker=(f"{STEP} checks={passed}/{total} state={state} version={VERSION} schema={SCHEMA} accepted_product_baseline={BASELINE} accepted_checks={BASELINE_CHECKS} correction=SHARED_STRUCTURED_MARKER_CONTRACT source=JSON_SINGLE_SOURCE validation=FIELD_SET_NOT_WHOLE_STRING retry=BOUNDED_TRANSPORT_ONLY timeout=CALLER_DEADLINE security=NON_TRANSPORT_FAIL_FAST delivery=DURABLE_TASK_EVENT semantics=REQUIRED_COMPLETION controller=OWNER_CONVERSATION_WAKE queue=SYSTEM_MESSAGE_WAKE_RUN restart=PENDING_DRAIN_IDENTITY_STABLE scope=CONTROLLER_TOOLS_DURABLE decision=EXPLICIT_TOOL_REQUIRED migration=TERMINAL_CHILD_SAFE_BACKFILL flow=CONTROLLER_OWNED_OUTCOME plan_executor=DEFERRED openclaw_reference=COMPLETION_DELIVERY_SOURCE_AUDITED external_model=NOT_RUN browser_live=NOT_RUN connector=DEFERRED_NO_REAL_SYSTEM focused_product={counts.get('focused-step020er2-product',0)} affected_regression={counts.get('affected-completion-regression',0)} governance={counts.get('focused-validation-governance',0)} canonical_files={counts.get('canonical-files',0)} canonical_tests={counts.get('canonical-suite',0)} windows_completion_marker_live={live} live_harness={LIVE_HARNESS} promotion={promotion} automated_run_seconds={seconds:.3f}")
    lines=[marker]+[f"OPENRILL_STEP020ER2_FAILURE check={name}\n{detail}" for name,ok,detail in checks if not ok]; write_acceptance_report(REPORT,"\n".join(lines)+"\n"); print(marker); [print(line) for line in lines[1:]]; return 0 if state=="PASSED" else 1
