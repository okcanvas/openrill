...
REPORT = resolve_acceptance_report(ROOT, ".artifacts/acceptance/STEP013CR2_ACCEPTANCE_REPORT.txt")
STAGE_LOG_DIR = REPORT.parent / "STEP013CR2_STAGES"
...
STAGE_TIMEOUTS = {
    ...
    "focused-sqlite-row-assertion": 120,
    "canonical-suite": 900,
    "architecture": 120,
    "exports": 180,
    "browser-live": 180,
...
def main() -> int:
    ...
    check("acceptance-script", scripts.get("acceptance:step013cr2") == "python scripts/run_step013cr2_acceptance.py")
    check("package-script", scripts.get("package:step013cr2") == "python scripts/package_step013cr2.py --output ../openrill-step013cr2-sqlite-null-prototype-live-assertion-alignment-v1.zip")
    ...
    required = [
        ...
        "scripts/run-step013cr2-host-child.mjs",
        "scripts/run-step013cr2-live.mjs",
        "scripts/run_step013cr2_acceptance.py",
        "scripts/package_step013cr2.py",
        "scripts/sh_run_step013cr2_acceptance.cmd",
        "scripts/sh_run_step013cr2_acceptance.sh",
        "scripts/recovery-live-assertions.mjs",
        "tests/unit/sqlite-null-prototype-live-assertion-step013cr2.test.mjs",
        ...
        "docs/plans/STEP013CR2_SQLITE_NULL_PROTOTYPE_LIVE_ASSERTION_ALIGNMENT.md",
        "docs/validation/STEP013CR2_FAILURE_PREVENTION_AUDIT.md",
        "reference/validation/STEP013B3_WINDOWS_LIVE_ACCEPTANCE.md",
        "reference/validation/STEP013CR2_LOCAL_VALIDATION.md",
        "reference/validation/STEP013CR1_WINDOWS_LIVE_NULL_PROTOTYPE_ASSERTION_FAILURE.md",
        "reference/validation/STEP013CR2_SQLITE_NULL_PROTOTYPE_ASSERTION_ALIGNMENT.md",
        "reference/validation/STEP013CR2_FOCUSED_STAGE_TIMEOUT_INVENTORY_OMISSION.md",
        "reference/validation/STEP013CR2_ACCEPTANCE_PREDICATE_OWNERSHIP_ALIGNMENT.md",
        "reference/validation/STEP013CR2_CURRENT_PACKAGE_MANIFEST_IDENTITY_FREEZE.md",
    ...
    package_manifest_payload = json.loads(read_utf8(ROOT / "PACKAGE_MANIFEST.json"))
    check("package-manifest-current-identity", package_manifest_payload.get("step") == STEP and package_manifest_payload.get("version") == VERSION, f"{package_manifest_payload.get('step')}:{package_manifest_payload.get('version')}")
    ...
    check("package-manifest-owner-sources", all(token in manifest_generator and token in manifest_verifier for token in (STEP, VERSION)))
    ...
    live = read_utf8(ROOT / "scripts/run-step013cr2-live.mjs")
    child = read_utf8(ROOT / "scripts/run-step013cr2-host-child.mjs")
    recovery_assertions = read_utf8(ROOT / "scripts/recovery-live-assertions.mjs")
    ...
    check("live-recovery-diagnostics", "OPENRILL_STEP013CR2_RECOVERY_DIAGNOSTICS" in live and all(token in live for token in ("latestEvents", "modelInvocations", "browserOperations")))
    ...
    cr1_windows_failure = read_utf8(ROOT / "reference/validation/STEP013CR1_WINDOWS_LIVE_NULL_PROTOTYPE_ASSERTION_FAILURE.md")
    check("cr1-windows-null-prototype-failure-evidence", "checks=139/140 state=FAILED" in cr1_windows_failure and "Object: null prototype" in cr1_windows_failure and "MODEL_INTERRUPTED_BY_RESTART" in cr1_windows_failure)
    ...
    check("live-model-interruption", "MODEL_INTERRUPTED_BY_RESTART" in recovery_assertions)
    check("live-prototype-neutral-invocation-assertion", "assertInterruptedModelInvocation(invocation)" in live and "deepEqual(invocation" not in live and "deepStrictEqual(invocation" not in live)
    check("assertion-field-values", all(token in recovery_assertions for token in ('assert.equal(row.status, "FAILED"', 'assert.equal(row.errorCode, "MODEL_INTERRUPTED_BY_RESTART"')) and "deepEqual" not in recovery_assertions and "deepStrictEqual" not in recovery_assertions)
    ...
    active_focused_commands = [
        ...
        "focused-browser-runtime", "focused-browser-boundaries", "focused-sqlite-row-assertion",
    ]
    source_self = read_utf8(ROOT / "scripts/run_step013cr2_acceptance.py")
    check("stage-output-full-log", "persist_stage_output(stage, result.output)" in source_self and "STEP013CR2_STAGES" in source_self)
    check("sqlite-row-stage-timeout", STAGE_TIMEOUTS.get("focused-sqlite-row-assertion") == 120)
    ...
    for stage_name in active_focused_commands:
        lines = [line for line in source_self.splitlines() if f'("{stage_name}"' in line and "--test-reporter=tap" in line]
        check(f"tap-reporter:{stage_name}", bool(lines))
    ...
    stages = [
        ...
        ("focused-sqlite-row-assertion", ["node", "--test", "--test-concurrency=1", "--test-reporter=tap", "tests/unit/sqlite-null-prototype-live-assertion-step013cr2.test.mjs"], lambda output: tap_pass(output, 7)),
        ...
        ("browser-live", ["node", "scripts/run-step013cr2-live.mjs"], lambda output: "OPENRILL_STEP013CR2_LIVE_PASS" in output and "automation=BROWSER_RUN ledger=ACTION_EVIDENCE recovery=RESUME_AND_REOPEN" in output and "process_count=0 chromium_orphan=0" in output),
    ...
    marker = (
        ...
        "sqlite_row=PROTOTYPE_NEUTRAL reporter=TAP process_count=0 chromium_orphan=0"
    )
    lines.append(marker)
    ...
    if state != "PASSED":
        for name, ok, detail in checks:
            if not ok:
                print(f"OPENRILL_STEP013CR2_FAILURE check={name}\n{detail}")
