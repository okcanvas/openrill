...
STEP = "STEP013AR4_ACCEPTANCE_STAGE_RUNNER_FIXTURE_IMPORT_ALIGNMENT"
VERSION = "0.13.4-step013ar4"
SCHEMA = 9
...
REPORT = resolve_acceptance_report(ROOT, ".artifacts/acceptance/STEP013AR4_ACCEPTANCE_REPORT.txt")
PACKAGED_REPORT = ROOT / "reference/validation/STEP013AR4_ACCEPTANCE_REPORT.txt"
...
STAGE_TIMEOUTS = {
    ...
    "focused-workspace-lock-alignment": 120,
    "focused-workspace-module-layout": 120,
    "package-manifest-initial": 120,
    "focused-build": 240,
    "focused-browser-runtime": 180,
    "focused-browser-boundaries": 180,
    "focused-historical-host-fixtures": 180,
    "focused-acceptance-stage-runner": 120,
    "focused-acceptance-stage-import": 120,
    "canonical-suite": 420,
...
SCAN_EXCLUDED_DIRS = {".git", "node_modules", "dist", ".artifacts"}


def source_tree_paths() -> list[Path]:
    result: list[Path] = []
    for current, directories, files in os.walk(ROOT, topdown=True, followlinks=False):
        directories[:] = [name for name in directories if name not in SCAN_EXCLUDED_DIRS and name != "__pycache__"]
        base = Path(current)
...
def clean() -> None:
    ...
    for current, directories, files in os.walk(ROOT, topdown=True, followlinks=False):
        directories[:] = [name for name in directories if name not in SCAN_EXCLUDED_DIRS]
        base = Path(current)
...
def main() -> int:
    ...
    check("step013ar4-script", scripts.get("acceptance:step013ar4") == "python scripts/run_step013ar4_acceptance.py")
    check("step013ar4-package-script", scripts.get("package:step013ar4") == "python scripts/package_step013ar4.py --output ../openrill-step013ar4-acceptance-stage-runner-fixture-import-alignment-v1.zip")
    ...
    required = [
        ...
        "scripts/verify_workspace_lock_alignment.py",
        "scripts/verify_workspace_module_links.py",
        "scripts/acceptance_stage_runner.py",
        "scripts/run_step013ar2_acceptance.py",
        "scripts/run_step013ar3_acceptance.py",
        "scripts/run_step013ar4_acceptance.py",
        "scripts/package_step013ar2.py",
        "scripts/package_step013ar3.py",
        "scripts/package_step013ar4.py",
        "scripts/sh_run_step013ar3_acceptance.cmd",
        "scripts/sh_run_step013ar3_acceptance.sh",
        "scripts/sh_run_step013ar4_acceptance.cmd",
        "scripts/sh_run_step013ar4_acceptance.sh",
        "tests/unit/workspace-lock-alignment-step013ar1.test.mjs",
        "tests/unit/workspace-module-link-layout-step013ar2.test.mjs",
        "tests/unit/acceptance-stage-runner-step013ar3.test.mjs",
        "tests/unit/acceptance-stage-runner-import-step013ar4.test.mjs",
        "docs/plans/STEP013AR2_WORKSPACE_MODULE_LINK_LAYOUT_ALIGNMENT.md",
        "docs/plans/STEP013AR4_ACCEPTANCE_STAGE_RUNNER_FIXTURE_IMPORT_ALIGNMENT.md",
        "reference/validation/STEP013A_WINDOWS_WORKSPACE_LOCK_IMPORTER_DRIFT.md",
        "reference/validation/STEP013AR1_HISTORICAL_FEATURE_CURRENT_RELEASE_VERSION_ASSERTION_DRIFT.md",
        "reference/validation/STEP013AR1_LOCAL_WORKSPACE_MODULE_LINK_CROSS_ROOT_CONTAMINATION.md",
        "reference/validation/STEP013AR1_WINDOWS_ROOT_WORKSPACE_SCOPE_LAYOUT_ASSUMPTION.md",
        "reference/validation/STEP013AR2_SUCCESS_REPORT_PHYSICAL_LAYOUT_DETAIL_DRIFT.md",
        "reference/validation/STEP013AR2_WINDOWS_ACCEPTANCE_SILENT_UNBOUNDED_STAGE_WAIT.md",
        "reference/validation/STEP013AR3_WINDOWS_STAGE_RUNNER_FIXTURE_IMPORT_PATH_DEPENDENCY.md",
        "reference/validation/STEP013AR4_ACCEPTANCE_REPORT.txt",
    ...
    host_importer = lock_text[lock_text.index("  services/agent-host:"):lock_text.index("  skills/builtin:")]
    workspace_config = read_utf8(ROOT / "pnpm-workspace.yaml")
    check("host-browser-runtime-lock-importer", all(token in host_importer for token in ("'@openrill/browser-runtime':", "specifier: workspace:*", "version: link:../../packages/browser-runtime")))
    check("pnpm-run-no-implicit-install", "verifyDepsBeforeRun: error" in workspace_config and "verifyDepsBeforeRun: install" not in workspace_config)
    ...
    issue_details = {
        ...
        "083": "reference/validation/STEP013A_WINDOWS_WORKSPACE_LOCK_IMPORTER_DRIFT.md",
        "084": "reference/validation/STEP013AR1_HISTORICAL_FEATURE_CURRENT_RELEASE_VERSION_ASSERTION_DRIFT.md",
        "085": "reference/validation/STEP013AR1_LOCAL_WORKSPACE_MODULE_LINK_CROSS_ROOT_CONTAMINATION.md",
        "086": "reference/validation/STEP013AR1_WINDOWS_ROOT_WORKSPACE_SCOPE_LAYOUT_ASSUMPTION.md",
        "087": "reference/validation/STEP013AR2_SUCCESS_REPORT_PHYSICAL_LAYOUT_DETAIL_DRIFT.md",
        "088": "reference/validation/STEP013AR2_WINDOWS_ACCEPTANCE_SILENT_UNBOUNDED_STAGE_WAIT.md",
        "089": "reference/validation/STEP013AR3_WINDOWS_STAGE_RUNNER_FIXTURE_IMPORT_PATH_DEPENDENCY.md",
    }
    ...
    plan = read_utf8(ROOT / "docs/plans/STEP013AR4_ACCEPTANCE_STAGE_RUNNER_FIXTURE_IMPORT_ALIGNMENT.md")
    for heading in (
        "## 목적", "## 기준선", "## 코드 확인", "## 구현 범위", "## 공개 계약",
    ...
    check("failure-prevention-audit", all(token in audit for token in ("single-flight", "final URL", "OR-ISSUE-081", "OR-ISSUE-083", "OR-ISSUE-085", "OR-ISSUE-086", "OR-ISSUE-087", "OR-ISSUE-088", "OR-ISSUE-089", "STEP013B")))
    ...
    version_ok, version_output = run_utf8(stage="source-version-alignment", command=["python", "scripts/verify_source_version_alignment.py"])
    ...
    cmd_bytes = (ROOT / "scripts/sh_run_step013ar4_acceptance.cmd").read_bytes()
    check("windows-launcher-crlf", b"\r\n" in cmd_bytes and b"\n" not in cmd_bytes.replace(b"\r\n", b""))
    check("windows-launcher-root-relative", b"%~dp0.." in cmd_bytes)
    check("posix-launcher", 'cd "$SCRIPT_DIR/.."' in read_utf8(ROOT / "scripts/sh_run_step013ar4_acceptance.sh"))

    lock_ok, lock_output = run_utf8(stage="workspace-lock-alignment", command=["python", "scripts/verify_workspace_lock_alignment.py"])
    check("workspace-lock-alignment", lock_ok and "OPENRILL_WORKSPACE_LOCK_ALIGNMENT_PASS importers=26 dependencies=64" in lock_output, lock_output.strip())

    links_ok, links_output = run_utf8(stage="workspace-module-links", command=["python", "scripts/verify_workspace_module_links.py"])
    links_contract = links_ok and "OPENRILL_WORKSPACE_MODULE_LINKS_PASS" in links_output and "root_owned=true" in links_output
    check("workspace-module-links-current-root", links_contract, "workspace_module_links_pass" if links_contract else links_output.strip())

    lock_test_ok, lock_test_output = run_utf8(stage="focused-workspace-lock-alignment", command=["node", "--test", "--test-concurrency=1", "--test-reporter=tap", "tests/unit/workspace-lock-alignment-step013ar1.test.mjs"])
    lock_test_contract = lock_test_ok and test_contract(lock_test_output, 4)
    check("focused-workspace-lock-alignment", lock_test_contract, "workspace_lock_alignment_pass" if lock_test_contract else stable_failure(lock_test_output))

    layout_test_ok, layout_test_output = run_utf8(stage="focused-workspace-module-layout", command=["node", "--test", "--test-concurrency=1", "--test-reporter=tap", "tests/unit/workspace-module-link-layout-step013ar2.test.mjs"])
    layout_test_contract = layout_test_ok and test_contract(layout_test_output, 3)
    check("focused-workspace-module-layout", layout_test_contract, "workspace_module_layout_pass" if layout_test_contract else stable_failure(layout_test_output))

    initial_ok, initial_output = run_utf8(stage="package-manifest-initial", command=["python", "scripts/verify_package_manifest.py"])
    check("package-manifest-initial", initial_ok and "OPENRILL_PACKAGE_MANIFEST_PASS" in initial_output, initial_output.strip())

    build_ok, build_output = run_utf8(stage="focused-build", command=["node", "scripts/workspace-runner.mjs", "build"])
    ...
    runtime_ok, runtime_output = run_utf8(stage="focused-browser-runtime", command=["node", "--test", "--test-concurrency=1", "--test-reporter=tap", "tests/unit/browser-runtime-step013a.test.mjs"])
    ...
    boundary_ok, boundary_output = run_utf8(stage="focused-browser-boundaries", command=["node", "--test", "--test-concurrency=1", "--test-reporter=tap", "tests/unit/browser-runtime-boundaries-step013a.test.mjs"])
    ...
    historical_ok, historical_output = run_utf8(stage="focused-historical-host-fixtures", command=["node", "--test", "--test-concurrency=1", "--test-reporter=tap", "tests/unit/automation-scheduler-step012b.test.mjs", "tests/unit/process-manager-close-step011r7.test.mjs"])
    ...
    stage_runner_ok, stage_runner_output = run_utf8(stage="focused-acceptance-stage-runner", command=["node", "--test", "--test-concurrency=1", "--test-reporter=tap", "tests/unit/acceptance-stage-runner-step013ar3.test.mjs"])
    stage_runner_contract = stage_runner_ok and test_contract(stage_runner_output, 4)
    check("focused-acceptance-stage-runner", stage_runner_contract, "acceptance_stage_runner_pass" if stage_runner_contract else stable_failure(stage_runner_output))

    stage_import_ok, stage_import_output = run_utf8(stage="focused-acceptance-stage-import", command=["node", "--test", "--test-concurrency=1", "--test-reporter=tap", "tests/unit/acceptance-stage-runner-import-step013ar4.test.mjs"])
    stage_import_contract = stage_import_ok and test_contract(stage_import_output, 2)
    check("focused-acceptance-stage-import", stage_import_contract, "acceptance_stage_import_pass" if stage_import_contract else stable_failure(stage_import_output))

    canonical_ok, canonical_output = run_utf8(stage="canonical-suite", command=["node", "scripts/run-step001-suite.mjs"])
    canonical_contract = canonical_ok and all(token in canonical_output for token in (
        "# tests 264", "# pass 264", "# fail 0", "# cancelled 0", "# skipped 0",
        "OPENRILL_ARCHITECTURE_PASS packages=25 edges=62 sources=105 ui_framework=VUE_3",
        "OPENRILL_PACKAGE_EXPORT_PASS packages=25",
        "OPENRILL_STEP001_SUITE_PASS unit_files=48 reporter=TAP concurrency=1",
    ))
    check("canonical-suite", canonical_contract, "suite_pass" if canonical_contract else stable_failure(canonical_output))

    final_ok, final_output = run_utf8(stage="package-manifest-final", command=["python", "scripts/verify_package_manifest.py"])
    ...
    clean_python_files = [path for path in source_tree_paths() if path.suffix in {".pyc", ".pyo"}]
    for path in clean_python_files:
        path.unlink(missing_ok=True)
    remaining_python_files = [path for path in source_tree_paths() if path.suffix in {".pyc", ".pyo"}]
    check("generated-cleanup", not remaining_python_files, json.dumps([path.relative_to(ROOT).as_posix() for path in remaining_python_files]))
    ...
    marker = (
        ...
        "retained_feature=STEP013A lock_importers=EXACT module_layout=RESOLUTION_AWARE pnpm_run=NO_IMPLICIT_INSTALL stages=VISIBLE_BOUNDED "
        "lifecycle=RUN_OWNED_GENERATION_GUARDED navigation=PRE_POST_POLICY shutdown=QUIESCENT "
        "tools=DEFERRED_STEP013B adapter=INJECTED"
    )
    lines.append(marker)
    report_text = "\n".join(lines) + "\n"
