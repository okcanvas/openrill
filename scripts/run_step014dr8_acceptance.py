...
STEP = "STEP014DR8_VUE_RUNTIME_MATERIALIZATION_AND_BROWSER_BOOTSTRAP_EVIDENCE_CLOSURE"
...
REPORT = resolve_acceptance_report(ROOT, ".artifacts/acceptance/STEP014DR8_ACCEPTANCE_REPORT.txt")
STAGE_LOG_DIR = REPORT.parent / "STEP014DR8_STAGES"
...
CANONICAL_EXPECTED = 467
VUE_WORK_ROOT = ROOT / ".artifacts/acceptance/STEP014DR8_VUE"
VUE_VENDOR_ROOT = VUE_WORK_ROOT / "vendor"
VUE_VERIFY_ROOT = VUE_WORK_ROOT / "verify"
VUE_ARCHIVE = VUE_WORK_ROOT / "vue-3.5.40.tgz"
VUE_VERIFY_ARCHIVE = VUE_WORK_ROOT / "vue-3.5.40-verify.tgz"
VENDOR_ENV_STAGES = {"focused-build", "deterministic-nested-control-ui-live"}


def vue_runtime_acquisition_command() -> list[str]:
    archive_input = os.environ.get("OPENRILL_VUE_ARCHIVE", "").strip()
    command = ["node", "scripts/vendor-vue-runtime.mjs"]
    command.extend(["--archive", archive_input] if archive_input else ["--download"])
    command.extend(["--output-root", str(VUE_VENDOR_ROOT), "--archive-output", str(VUE_ARCHIVE)])
    return command
...
STAGES: list[tuple[str, list[str], int]] = [
    ...
    ("vue-runtime-acquisition", vue_runtime_acquisition_command(), 120),
    ("vue-runtime-reextract", ["node", "scripts/vendor-vue-runtime.mjs", "--archive", str(VUE_ARCHIVE), "--output-root", str(VUE_VERIFY_ROOT), "--archive-output", str(VUE_VERIFY_ARCHIVE)], 120),
    ("vue-runtime-byte-verification", ["node", "scripts/verify-step014dr8-vue-runtime.mjs", "--vendor-root", str(VUE_VENDOR_ROOT), "--verify-root", str(VUE_VERIFY_ROOT), "--archive", str(VUE_ARCHIVE), "--verify-archive", str(VUE_VERIFY_ARCHIVE)], 60),
    ...
    ("focused-step014dr6-determinism", ["node", "--test", "--test-reporter=tap", "tests/unit/step014dr6-acceptance-determinism.test.mjs"], 180),
    ("focused-step014dr6-boundaries", ["node", "--test", "--test-reporter=tap", "tests/unit/step014dr6-boundaries.test.mjs"], 180),
    ("focused-live-loopback-http", ["node", "--test", "--test-reporter=tap", "tests/unit/live-loopback-http.test.mjs"], 180),
    ("focused-step014dr7-closure", ["node", "--test", "--test-reporter=tap", "tests/unit/step014dr7-live-acceptance-closure.test.mjs"], 180),
    ("focused-step014dr7-boundaries", ["node", "--test", "--test-reporter=tap", "tests/unit/step014dr7-boundaries.test.mjs"], 180),
    ("focused-canonical-batch-runner", ["node", "--test", "--test-reporter=tap", "tests/unit/canonical-batch-runner-step014dr7.test.mjs"], 180),
    ("focused-step014dr8-vue-browser-closure", ["node", "--test", "--test-reporter=tap", "tests/unit/step014dr8-vue-browser-closure.test.mjs"], 180),
    ("live-acceptance-lifecycle-audit", ["python", "scripts/check_live_acceptance_lifecycle.py"], 120),
    ("canonical-suite", ["node", "scripts/run-canonical-unit-batches.mjs", "--expected-tests", str(CANONICAL_EXPECTED), "--batch-size", "16"], 900),
    ("architecture", ["python", "scripts/check_architecture.py"], 120),
    ("exports", ["node", "scripts/check-exports.mjs"], 180),
    ("external-model-parallel-live", ["node", "scripts/run-step014dr8-external-model-live.mjs"], 300),
    ("deterministic-nested-control-ui-live", ["node", "scripts/run-step014dr8-deterministic-nested-ui-live.mjs"], 180),
...
def run_utf8(name: str, command: list[str], timeout: int) -> tuple[bool, str]:
    ...
    if name in VENDOR_ENV_STAGES:
        env["OPENRILL_VUE_RUNTIME_VENDOR_DIR"] = str(VUE_VENDOR_ROOT)
...
def main() -> int:
    ...
    check("current-acceptance-script", scripts.get("acceptance:step014dr8") == "python scripts/run_step014dr8_acceptance.py")
    check("current-package-script", scripts.get("package:step014dr8") == "python scripts/package_step014dr8.py --output ../openrill-step014dr8-vue-runtime-materialization-browser-bootstrap-evidence-closure-v1.zip")
    check("retained-dr6-acceptance-script", scripts.get("acceptance:step014dr6") == "python scripts/run_step014dr6_acceptance.py")
    check("retained-dr6-package-script", scripts.get("package:step014dr6") == "python scripts/package_step014dr6.py --output ../openrill-step014dr6-external-model-acceptance-determinism-nested-ui-evidence-separation-v1.zip")
    ...
    required = [
        ...
        "tests/unit/delegation-control-step014d.test.mjs", "tests/unit/delegation-control-boundaries-step014d.test.mjs", "tests/unit/model-openai-responses-step014dr2.test.mjs", "tests/unit/step014dr2-boundaries.test.mjs", "tests/unit/model-openai-responses-step014dr3.test.mjs", "tests/unit/step014dr3-boundaries.test.mjs", "tests/unit/delegation-default-budget-step014dr4.test.mjs", "tests/unit/step014dr4-boundaries.test.mjs", "tests/unit/control-ui-entrypoint-step014dr5.test.mjs", "tests/unit/step014dr5-boundaries.test.mjs", "tests/unit/step014dr6-acceptance-determinism.test.mjs", "tests/unit/step014dr6-boundaries.test.mjs",
        "scripts/run-step014d-live.mjs", "scripts/run-step014dr1-live.mjs", "scripts/run-step014dr2-live.mjs", "scripts/run-step014dr4-live.mjs", "scripts/run-step014dr5-live.mjs", "scripts/run-step014dr6-live.mjs", "scripts/run-step014dr6-external-model-live.mjs", "scripts/run-step014dr6-deterministic-nested-ui-live.mjs", "scripts/step014dr6-deterministic-nested-fixture.mjs", "scripts/control-ui-static-contract.mjs", "scripts/step014dr1-live-diagnostics.mjs",
        "scripts/check_source_root_boundary.py", "scripts/run_step014dr1_acceptance.py", "scripts/package_step014dr1.py", "scripts/run_step014dr2_acceptance.py", "scripts/package_step014dr2.py", "scripts/run_step014dr4_acceptance.py", "scripts/package_step014dr4.py", "scripts/run_step014dr6_acceptance.py", "scripts/package_step014dr6.py",
        "scripts/sh_run_step014d_acceptance.cmd", "scripts/sh_run_step014d_acceptance.sh",
        "scripts/sh_run_step014dr1_acceptance.cmd", "scripts/sh_run_step014dr1_acceptance.sh", "scripts/sh_run_step014dr2_acceptance.cmd", "scripts/sh_run_step014dr2_acceptance.sh", "scripts/sh_run_step014dr3_acceptance.cmd", "scripts/sh_run_step014dr3_acceptance.sh", "scripts/sh_run_step014dr4_acceptance.cmd", "scripts/sh_run_step014dr4_acceptance.sh", "scripts/sh_run_step014dr5_acceptance.cmd", "scripts/sh_run_step014dr5_acceptance.sh", "scripts/sh_run_step014dr6_acceptance.cmd", "scripts/sh_run_step014dr6_acceptance.sh",
        "docs/plans/STEP014DR5_CONTROL_UI_ENTRYPOINT_DISCOVERY_AND_STATIC_ASSET_ROUTE_ALIGNMENT.md", "docs/plans/STEP014DR8_VUE_RUNTIME_MATERIALIZATION_AND_BROWSER_BOOTSTRAP_EVIDENCE_CLOSURE.md",
        "docs/adrs/ADR-0030-SEPARATE_STOCHASTIC_MODEL_BEHAVIOR_FROM_DETERMINISTIC_RUNTIME_ACCEPTANCE.md", "docs/validation/STEP014DR6_FAILURE_PREVENTION_AUDIT.md", "docs/contracts/DELEGATED_WORK_CONTROL_SURFACE.md", "docs/contracts/MODEL_ADAPTER.md", "docs/adrs/ADR-0033-PROVIDER_SAFE_TOOL_ALIASES_WITH_CANONICAL_RUNTIME_NAMES.md", "docs/adrs/ADR-0034-OPENAI_RESPONSES_ITEM_AND_CALL_ID_UNIFICATION.md", "docs/adrs/ADR-0035-FAIR_SHARE_DELEGATION_DEFAULT_RESERVATIONS.md", "docs/validation/STEP014DR2_FAILURE_PREVENTION_AUDIT.md", "docs/validation/STEP014DR3_FAILURE_PREVENTION_AUDIT.md", "docs/validation/STEP014DR4_FAILURE_PREVENTION_AUDIT.md", "docs/validation/STEP014DR6_FAILURE_PREVENTION_AUDIT.md",
        ...
        "reference/validation/STEP014DR1_LOCAL_DETERMINISTIC_VALIDATION.md", "reference/validation/STEP014DR2_LOCAL_DETERMINISTIC_VALIDATION.md", "reference/validation/STEP014DR3_LOCAL_DETERMINISTIC_VALIDATION.md", "reference/validation/STEP014DR4_LOCAL_DETERMINISTIC_VALIDATION.md", "reference/validation/STEP014DR6_LOCAL_DETERMINISTIC_VALIDATION.md",
        ...
        "scripts/live-loopback-http.mjs", "scripts/check_live_acceptance_lifecycle.py", "scripts/browser-page-evidence.mjs", "scripts/run-canonical-unit-batches.mjs",
        "scripts/run-step014dr8-external-model-live.mjs", "scripts/run-step014dr8-deterministic-nested-ui-live.mjs", "scripts/step014dr6-deterministic-nested-fixture.mjs",
        "scripts/run_step014dr7_acceptance.py", "scripts/package_step014dr7.py", "scripts/sh_run_step014dr7_acceptance.cmd", "scripts/sh_run_step014dr7_acceptance.sh",
        "scripts/run_step014dr8_acceptance.py", "scripts/package_step014dr8.py", "scripts/sh_run_step014dr8_acceptance.cmd", "scripts/sh_run_step014dr8_acceptance.sh", "scripts/verify-step014dr8-vue-runtime.mjs",
        "tests/unit/live-loopback-http.test.mjs", "tests/unit/canonical-batch-runner-step014dr7.test.mjs", "tests/unit/step014dr7-live-acceptance-closure.test.mjs", "tests/unit/step014dr7-boundaries.test.mjs", "tests/unit/step014dr8-vue-browser-closure.test.mjs",
        "reference/validation/STEP014_ACCEPTANCE_CLOSURE_AUDIT.md", "docs/plans/STEP014DR8_VUE_RUNTIME_MATERIALIZATION_AND_BROWSER_BOOTSTRAP_EVIDENCE_CLOSURE.md", "docs/validation/STEP014DR8_FAILURE_PREVENTION_AUDIT.md", "reference/validation/STEP014DR8_LOCAL_DETERMINISTIC_VALIDATION.md",
        "config/current-accepted-baseline.json",
    ] + [f"reference/validation/STEP014D_OR_ISSUE_{number}.md" for number in range(146, 157)] + [f"reference/validation/STEP014DR1_OR_ISSUE_{number}.md" for number in range(157, 161)] + [f"reference/validation/STEP014DR2_OR_ISSUE_{number}.md" for number in range(161, 164)] + [f"reference/validation/STEP014DR4_OR_ISSUE_{number}.md" for number in range(168, 173)] + ["reference/validation/STEP014DR5_OR_ISSUE_173.md", "reference/validation/STEP014DR6_OR_ISSUE_174.md", "reference/validation/STEP014DR6_OR_ISSUE_175.md"] + [f"reference/validation/STEP014DR7_OR_ISSUE_{number}.md" for number in range(176, 184)] + [f"reference/validation/STEP014DR8_OR_ISSUE_{number}.md" for number in range(184, 190)]
    ...
    check("control-ui-live-discovers-entrypoint", "controlUiModuleEntrypointFromHtml(indexResponse.text)" in live and "new URL(servedEntrypoint,uiBase)" in live and "/assets/app.js" not in live)

    external_live = read_utf8("scripts/run-step014dr8-external-model-live.mjs")
    nested_ui_live = read_utf8("scripts/run-step014dr8-deterministic-nested-ui-live.mjs")
    check("dr6-external-direct-parallel", "Call agent.spawn twice without waiting" in external_live and "direct.length>=2" in external_live and "grandchild" not in external_live)
    check("dr6-nested-deterministic-ui", "seedDeterministicNestedDelegationFixture" in nested_ui_live and re.search(r"depth\s*===\s*2", nested_ui_live) and "resolveChromiumExecutable" in nested_ui_live)
    check("dr6-live-separation", 'required("OPENAI_API_KEY")' in external_live and 'required("OPENAI_API_KEY")' not in nested_ui_live)
    check("dr8-live-client-version", all(VERSION in body and "0.14.10-step014dr7" not in body and "0.14.9-step014dr7" not in body for body in (external_live, nested_ui_live)))
    check("dr8-retained-nested-fixture-owner", './step014dr6-deterministic-nested-fixture.mjs' in nested_ui_live and './step014dr8-deterministic-nested-fixture.mjs' not in nested_ui_live and (ROOT / "scripts/step014dr6-deterministic-nested-fixture.mjs").is_file())
    check("dr8-vue-runtime-preflight", all(token in nested_ui_live for token in ("verifyServedVueRuntime", "OPENRILL_VUE_RUNTIME_VENDOR_DIR", "vue_runtime=VERIFIED")))
    vue_verifier = read_utf8("scripts/verify-step014dr8-vue-runtime.mjs")
    check("dr8-vue-lock-contract", all(token in vue_verifier for token in ("primary.lock.schemaVersion", 'primary.lock.package, "vue"', "primary.lock.archiveFile", "primary.lock.licenseFile", "assert.deepEqual(verify.lockBytes, primary.lockBytes)")))
    check("dr8-browser-bootstrap-evidence", all(token in nested_ui_live for token in ("attachBrowserPageEvidence", "enableBrowserPageEvidence", "waitForBrowserCondition", "navigation.errorText", "OPENRILL_STEP014DR8_UI_NAVIGATION_FAILED", "browser_evidence=CLEAN")))
    check("dr8-partial-browser-launch-cleanup", all(token in nested_ui_live for token in ("await closeBrowser({ child, cdp })", "AggregateError", "OPENRILL_STEP014DR8_BROWSER_LAUNCH_CLEANUP_FAILED", "chromium-taskkill-exit")))
    check("dr8-final-cleanup-failure-preservation", all(token in nested_ui_live for token in ("let primaryError;", "const cleanupFailures = []", "OPENRILL_STEP014DR8_BODY_AND_CLEANUP_FAILED", "OPENRILL_STEP014DR8_CLEANUP_FAILED")) and "await closeBrowser(browser).catch(() => undefined)" not in nested_ui_live and 'await host?.close("step014dr8-ui-live").catch(() => undefined)' not in nested_ui_live)
    stage_names = [name for name, _, _ in STAGES]
    check("dr8-vendor-stage-order", stage_names.index("vue-runtime-acquisition") < stage_names.index("vue-runtime-reextract") < stage_names.index("vue-runtime-byte-verification") < stage_names.index("focused-build") < stage_names.index("deterministic-nested-control-ui-live"))
    check("dr8-vendor-env-propagation", VENDOR_ENV_STAGES == {"focused-build", "deterministic-nested-control-ui-live"} and "OPENRILL_VUE_RUNTIME_VENDOR_DIR" in read_utf8("scripts/run_step014dr8_acceptance.py"))
    loopback_http = read_utf8("scripts/live-loopback-http.mjs")
    lifecycle_audit = read_utf8("scripts/check_live_acceptance_lifecycle.py")
    check("dr7-bounded-loopback-http", all(token in loopback_http for token in ("agent: false", "request.setTimeout", "LIVE_HTTP_BODY_TOO_LARGE", "OPENRILL_LIVE_HTTP_START", "OPENRILL_LIVE_HTTP_END")))
    check("dr7-response-body-drained", "getLoopbackText(new URL(entry, uiBase)" in nested_ui_live and "module.text.includes" in nested_ui_live)
    check("dr7-lifecycle-audit", "HTTP_FIXTURES" in lifecycle_audit and "CHROMIUM_FIXTURES" in lifecycle_audit and "OPENRILL_LIVE_ACCEPTANCE_LIFECYCLE_AUDIT_PASS" in lifecycle_audit)
    check("dr8-current-lifecycle-audit-inventory", all(token in lifecycle_audit for token in ("run-step014dr8-external-model-live.mjs", "run-step014dr8-deterministic-nested-ui-live.mjs", "current-partial-launch-cleanup")))
    canonical_batches = read_utf8("scripts/run-canonical-unit-batches.mjs")
    check("dr7-canonical-batches", all(token in canonical_batches for token in ("OPENRILL_CANONICAL_BATCHES_PASS", "OPENRILL_CANONICAL_FILE_START", "OPENRILL_CANONICAL_FILE_END", "NODE_TEST_CONTEXT", "--test-concurrency=1", "async function runFile")))
    check("dr7-product-surface-frozen", "OPENRILL_STATE_SCHEMA_VERSION = 14 as const" in read_utf8("packages/state/src/migrations.ts"))
    ...
    stage_expectations = {
        ...
        "focused-step014dr6-determinism": 4,
        "focused-step014dr6-boundaries": 4,
        "focused-live-loopback-http": 4,
        "focused-step014dr7-closure": 5,
        "focused-step014dr7-boundaries": 4,
        "focused-canonical-batch-runner": 2,
        "focused-step014dr8-vue-browser-closure": 4,
    }
    for name, command, timeout in STAGES:
        ...
        if name == "canonical-suite":
            ok = ok and f"OPENRILL_CANONICAL_BATCHES_PASS files={len(UNIT_TEST_FILES)}" in output and f"tests={CANONICAL_EXPECTED} pass={CANONICAL_EXPECTED} fail=0 skipped=0" in output
        if name == "external-model-parallel-live":
            ok = ok and "STEP014DR8_EXTERNAL_MODEL_PARALLEL_PASS" in output
        if name == "deterministic-nested-control-ui-live":
            ok = ok and "STEP014DR8_DETERMINISTIC_NESTED_UI_PASS" in output and "vue_runtime=VERIFIED" in output and "browser_evidence=CLEAN" in output and "chromium_orphan=0" in output
    ...
    marker = (
        ...
        "external_model=PARALLEL_DIRECT nested=DETERMINISTIC_RUNTIME_UI chromium=RENDERED privacy=BOUNDED diagnostics=TYPED_AND_PRESERVED source_root=ARCHIVE_FREE provider_tool_names=ALIASED canonical_tools=ROUND_TRIP stream_tool_identity=UNIFIED empty_tool_name=FAIL_CLOSED delegation_defaults=PARALLEL_NESTED_FIT tool_errors=TYPED_AND_PRESERVED ui_entrypoint=DISCOVERED_AND_ALIGNED acceptance_evidence=STOCHASTIC_DETERMINISTIC_SEPARATED acceptance_transport=BOUNDED_NODE_HTTP lifecycle=CLOSURE_AUDITED vue_runtime=MATERIALIZED_AND_SERVED browser_bootstrap=EVIDENCE_PRESERVED reporter=TAP chromium_orphan=0"
    )
    lines = [marker]
    for name, ok, detail in checks:
        if not ok:
            lines.append(f"OPENRILL_STEP014DR8_FAILURE check={name}")
            if detail:
                lines.append(detail)
