...
STEP = "STEP013AR2_WORKSPACE_MODULE_LINK_LAYOUT_ALIGNMENT"
...
REPORT = resolve_acceptance_report(ROOT, ".artifacts/acceptance/STEP013AR2_ACCEPTANCE_REPORT.txt")
PACKAGED_REPORT = ROOT / "reference/validation/STEP013AR2_ACCEPTANCE_REPORT.txt"
...
def main() -> int:
    ...
    check("step013ar2-script", scripts.get("acceptance:step013ar2") == "python scripts/run_step013ar2_acceptance.py")
    check("step013ar2-package-script", scripts.get("package:step013ar2") == "python scripts/package_step013ar2.py --output ../openrill-step013ar2-workspace-module-link-layout-alignment-v1.zip")
    ...
    required = [
        ...
        "scripts/sh_run_step013ar2_acceptance.cmd",
        "scripts/sh_run_step013ar2_acceptance.sh",
        ...
        "reference/validation/STEP013AR2_ACCEPTANCE_REPORT.txt",
        "scripts/sh_run_step013ar2_acceptance.cmd",
        "scripts/sh_run_step013ar2_acceptance.sh",
    ...
    plan = read_utf8(ROOT / "docs/plans/STEP013AR2_WORKSPACE_MODULE_LINK_LAYOUT_ALIGNMENT.md")
    for heading in (
    ...
    check("failure-prevention-audit", all(token in audit for token in ("single-flight", "final URL", "OR-ISSUE-081", "OR-ISSUE-083", "OR-ISSUE-085", "OR-ISSUE-086", "OR-ISSUE-087", "STEP013B")))
    ...
    cmd_bytes = (ROOT / "scripts/sh_run_step013ar2_acceptance.cmd").read_bytes()
    check("windows-launcher-crlf", b"\r\n" in cmd_bytes and b"\n" not in cmd_bytes.replace(b"\r\n", b""))
    check("windows-launcher-root-relative", b"%~dp0.." in cmd_bytes)
    check("posix-launcher", 'cd "$SCRIPT_DIR/.."' in read_utf8(ROOT / "scripts/sh_run_step013ar2_acceptance.sh"))

    lock_ok, lock_output = run_utf8(["python", "scripts/verify_workspace_lock_alignment.py"])
    ...
    links_ok, links_output = run_utf8(["python", "scripts/verify_workspace_module_links.py"])
    ...
    lock_test_ok, lock_test_output = run_utf8(["node", "--test", "--test-concurrency=1", "--test-reporter=tap", "tests/unit/workspace-lock-alignment-step013ar1.test.mjs"])
    ...
    layout_test_ok, layout_test_output = run_utf8(["node", "--test", "--test-concurrency=1", "--test-reporter=tap", "tests/unit/workspace-module-link-layout-step013ar2.test.mjs"])
    ...
    marker = (
        ...
        "retained_feature=STEP013A lock_importers=EXACT module_layout=RESOLUTION_AWARE pnpm_run=NO_IMPLICIT_INSTALL "
