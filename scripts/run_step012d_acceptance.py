...
STEP = "STEP012D_AUTOMATION_CONTROL_UI_WINDOWS_VERTICAL_SLICE"
VERSION = "0.12.6-step012d"
...
REPORT = resolve_acceptance_report(ROOT, ".artifacts/acceptance/STEP012D_ACCEPTANCE_REPORT.txt")
PACKAGED_REPORT = ROOT / "reference/validation/STEP012D_ACCEPTANCE_REPORT.txt"
VUE_VERSION = "3.5.40"
...
def main() -> int:
    ...
    check("step012d-script", scripts.get("acceptance:step012d") == "python scripts/run_step012d_acceptance.py")
    check("step012d-package-script", scripts.get("package:step012d") == "python scripts/package_step012d.py --output ../openrill-step012d-automation-control-ui-windows-vertical-slice-v1.zip")

    required = [
        "apps/agent-web/src/browser-app.ts", "apps/agent-web/public/assets/app.css",
        "scripts/run-step012d-live.mjs", "scripts/run_step012d_acceptance.py",
        "scripts/sh_run_step012d_acceptance.cmd", "scripts/sh_run_step012d_acceptance.sh", "scripts/package_step012d.py",
        "tests/unit/automation-control-ui-step012d.test.mjs",
        "docs/plans/STEP012D_AUTOMATION_CONTROL_UI_WINDOWS_VERTICAL_SLICE.md",
        ...
        "reference/validation/STEP012D_ACCEPTANCE_REPORT.txt",
    ...
    plan = read_utf8(ROOT / "docs/plans/STEP012D_AUTOMATION_CONTROL_UI_WINDOWS_VERTICAL_SLICE.md")
    ...
    cmd_bytes = (ROOT / "scripts/sh_run_step012d_acceptance.cmd").read_bytes()
    check("windows-launcher-crlf", b"\r\n" in cmd_bytes and b"\n" not in cmd_bytes.replace(b"\r\n", b""))
    check("windows-launcher-root-relative", b"%~dp0.." in cmd_bytes)
    check("posix-launcher", 'cd "$SCRIPT_DIR/.."' in read_utf8(ROOT / "scripts/sh_run_step012d_acceptance.sh"))
    ...
    ui_contract = bool(ui_ok and re.search(r"# tests 5(?:\r?\n)", ui_output) and re.search(r"# pass 5(?:\r?\n)", ui_output) and re.search(r"# fail 0(?:\r?\n)", ui_output) and re.search(r"# skipped 0(?:\r?\n)", ui_output))
    check("focused-step012d-ui", ui_contract, "step012d_ui_tests_pass" if ui_contract else stable_failure(ui_output))
    ...
    if vendor_ok:
        ...
        try:
            ...
            if not vendor_failures and suite_contract:
                live_ok, live_output = run_utf8(["node", "scripts/run-step012d-live.mjs"], env={"OPENRILL_VUE_RUNTIME_VENDOR_DIR": str(vendor_root)})
    ...
    check("step012d-exact-vue-actual-chromium", browser_contract, browser_detail)
    ...
    lines.append(f"{STEP} checks={passed}/{len(checks)} state={state} schema={SCHEMA} ui=AUTOMATION_CRUD_RUN_HISTORY notices=JOB_RUN_CANONICAL_REFRESH manual_replay=DURABLE interval_anchor=PRESERVED browser=CHROMIUM mobile=PASS")
