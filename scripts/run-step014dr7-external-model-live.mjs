...
const root=await mkdtemp(join(tmpdir(),"openrill-step014dr7-model-live-"));
const workspace=join(root,"workspace");await mkdir(workspace,{recursive:true});
const profile="step014dr7-model-live";
...
try{
  ...
  const bootstrap=(await getLoopbackJson(`http://127.0.0.1:${host.port}/ui/bootstrap`,{label:"step014dr7-model-bootstrap",expectedStatus:200,maxBytes:1024*1024})).json;
  client=new LocalProtocolClient({url:`ws://127.0.0.1:${host.port}/protocol`,token:bootstrap.protocol.token,clientId:"step014dr7-model-live",clientVersion:"0.14.10-step014dr7",platform:process.platform});
  ...
  const conversation=await client.call("conversation.create",{workspaceId:"alpha",title:"STEP014DR7 external parallel delegated work"},"step014dr7:create");
  const prompt=[
  ...
  const sent=await client.call("conversation.send",{workspaceId:"alpha",conversationId:conversation.conversationId,submissionKey:"step014dr7:send",text:prompt},"step014dr7:send");rootRunId=sent.run.runId;
  let view;let items=[];const deadline=Date.now()+180_000;
  while(Date.now()<deadline){view=await client.call("conversation.get",{workspaceId:"alpha",conversationId:conversation.conversationId},`step014dr7:get:${Date.now()}`);items=(await client.call("delegation.list",{rootRunId,limit:20},`step014dr7:list:${Date.now()}`)).items;const run=view.runs.find(item=>item.runId===rootRunId);if(run?.status==="COMPLETED"&&items.length>=2&&items.every(item=>["COMPLETED","FAILED","CANCELLED","TIMED_OUT"].includes(item.status)))break;if(run&&["FAILED","CANCELLED"].includes(run.status))throw new Error(`OPENRILL_STEP014DR7_ROOT_RUN_FAILED:${JSON.stringify({status:run.status,items:items.map(item=>({depth:item.depth,status:item.status,errorCode:item.errorCode}))})}`);await wait(500);}
