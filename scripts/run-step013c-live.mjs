...
const marker = `openrill-step013c-${randomUUID()}`;
const profile = "step013c-live";
...
function toolEvents(requestNumber, name, args) {
  const callId = `step013c-call-${requestNumber}-${name.replace(/[^a-z]/g, "-")}`;
  return [
...
function finalEvents(requestNumber) {
  return [
    { type: "response.created", response: { id: `response-${requestNumber}` } },
    { type: "response.output_text.delta", delta: "STEP013C autonomous browser run completed" },
    { type: "response.completed", response: { id: `response-${requestNumber}`, usage: { input_tokens: 1, output_tokens: 1, total_tokens: 2 } } },
  ];
}
...
async function startFixtureServer() {
  const server = createServer((request, response) => {
    ...
    if (url.pathname === "/page") {
      ...
      response.end(`<!doctype html><html><head><title>STEP013C fixture</title></head><body><h1>Automation Browser</h1><p>restart recovery</p><script>console.log("step013c-console");fetch("/api?token=secret-step013c");</script></body></html>`);
      return;
    }
    if (url.pathname === "/api") {
...
function spawnChild(env) {
  const child = spawn(process.execPath, [join(import.meta.dirname, "run-step013c-host-child.mjs")], { cwd: join(import.meta.dirname, ".."), env, stdio: ["pipe", "pipe", "pipe"], windowsHide: true });
  let output = "";
...
const root = await mkdtemp(join(tmpdir(), "openrill-step013c-live-"));
...
try {
  ...
  const job = definitions.create({
    name: "STEP013C autonomous browser",
    enabled: false,
  ...
  const scheduled = definitions.runNow(job.jobId, "step013c-live-request").run;
  ...
  const childEnv = {
    ...process.env,
    ...envRoot,
    OPENRILL_STEP013C_PROFILE: profile,
    OPENRILL_STEP013C_WORKSPACE: workspacePath,
    OPENRILL_STEP013C_MODEL_ENDPOINT: model.endpoint,
    OPENRILL_STEP013C_BROWSER_MARKER: marker,
  };
  ...
  const preCrashMessages = queryAll(databasePath, "SELECT role,content_json contentJson FROM conversation_messages WHERE conversation_id=(SELECT conversation_id FROM agent_runs WHERE run_id=?) ORDER BY sequence", runId);
  assert.equal(
    preCrashOperations.filter((row) => row.toolName === "browser.open" && row.status === "SUCCEEDED").length,
    1,
    `browser.open ledger missing; operations=${JSON.stringify(preCrashOperations)} messages=${JSON.stringify(preCrashMessages)}`,
  );
  ...
  assert.deepEqual(invocation, { status: "FAILED", errorCode: "MODEL_INTERRUPTED_BY_RESTART" });
  ...
  assert.doesNotMatch(durableText, /secret-step013c/);
  ...
  console.log(`OPENRILL_STEP013C_LIVE_PASS automation=BROWSER_RUN ledger=ACTION_EVIDENCE recovery=RESUME_AND_REOPEN model_requests=${model.requestCount()} process_count=0 chromium_orphan=0`);
} catch (error) {
  primaryError = error;
} finally {
  ...
  if (cleanupFailures.length > 0) {
    ...
    if (primaryError) process.stderr.write(`OPENRILL_STEP013C_CLEANUP_AFTER_FAILURE ${detail}\n`);
    else primaryError = new AggregateError(cleanupFailures, `STEP013C live cleanup failed: ${detail}`);
  }
}
