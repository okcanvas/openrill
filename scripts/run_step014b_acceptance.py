...
STEP = "STEP014B_SINGLE_CHILD_DELEGATED_EXECUTION_AND_DURABLE_PARENT_RESUME"
...
REPORT = resolve_acceptance_report(ROOT, ".artifacts/acceptance/STEP014B_ACCEPTANCE_REPORT.txt")
STAGE_LOG_DIR = REPORT.parent / "STEP014B_STAGES"
...
def main() -> int:
    ...
    check("acceptance-script", scripts.get("acceptance:step014b") == "python scripts/run_step014b_acceptance.py")
    check("package-script", scripts.get("package:step014b") == "python scripts/package_step014b.py --output ../openrill-step014b-single-child-delegated-execution-durable-parent-resume-v1.zip")
    ...
    required = [
        ...
        "docs/plans/STEP014B_SINGLE_CHILD_DELEGATED_EXECUTION_AND_DURABLE_PARENT_RESUME.md",
        ...
        "scripts/run_step014b_acceptance.py", "scripts/package_step014b.py",
        "scripts/sh_run_step014b_acceptance.cmd", "scripts/sh_run_step014b_acceptance.sh",
        "config/current-accepted-baseline.json",
    ] + [f"reference/validation/STEP014B_OR_ISSUE_{number}.md" for number in range(129, 137)]
    ...
    check("schema-13", f"OPENRILL_STATE_SCHEMA_VERSION = {SCHEMA} as const" in migrations)
    ...
    marker = (
        ...
        "delegation=SINGLE_CHILD tools=AGENT_SPAWN_WAIT wait=DURABLE result=EXACTLY_ONCE "
        "resume=PARENT_ATTEMPT_2 scope=MONOTONIC protocol=UNCHANGED reporter=TAP"
    )
    lines = [marker]
    for name, ok, detail in checks:
        if not ok:
            lines.append(f"OPENRILL_STEP014B_FAILURE check={name}")
            if detail:
                lines.append(detail)
