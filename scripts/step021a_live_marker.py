...
CONTRACT_PATH=ROOT/'config/step021a-live-marker-contract.json'
def load_contract(path:Path=CONTRACT_PATH)->dict:
    value=json.loads(path.read_text(encoding='utf-8'))
    if not isinstance(value,dict) or not isinstance(value.get('fields'),dict): raise ValueError('OPENRILL_STEP021A_LIVE_MARKER_CONTRACT_INVALID')
    return value
