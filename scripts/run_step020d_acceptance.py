...
STEP="STEP020D_TASK_AND_TASK_FLOW_RECONCILIATION_LOST_AND_RETENTION_FOUNDATION"; VERSION="0.20.4-step020d"; SCHEMA=21
LIVE_HARNESS="STEP020D_H1_TASK_FLOW_MAINTENANCE_RECONCILIATION_LOST_AND_RETENTION"
BASELINE="STEP020C_BOUND_TASK_FLOW_CONTROLLER_RUNTIME_AND_ATOMIC_CHILD_TASK_ADMISSION"; BASELINE_CHECKS="43/43"; BASELINE_SHA="7aa16634fb7ef9fcbf498e8dd7e200c29ba1cf403dc5de870e6359035c0a3ca5"
REPORT=resolve_acceptance_report(ROOT,".artifacts/acceptance/STEP020D_ACCEPTANCE_REPORT.txt"); LOGDIR=REPORT.parent/"STEP020D_STAGES"
PRODUCT=[
...
GOVERNANCE=[
 ...
 "tests/unit/validation-governance-step020a.test.mjs","tests/unit/validation-governance-step020b.test.mjs","tests/unit/validation-governance-step020br1.test.mjs","tests/unit/validation-governance-step020c.test.mjs","tests/unit/validation-governance-step020d.test.mjs",
]
STAGES=[
 ...
 ("focused-step020d-product",["node","--test","--test-concurrency=1","--test-reporter=tap",*PRODUCT],500),
 ("affected-maintenance-regression",["node","--test","--test-concurrency=1","--test-reporter=tap",*AFFECTED],900),
...
def main():
    parser=argparse.ArgumentParser(); parser.add_argument("--require-windows-maintenance-live",action="store_true"); args=parser.parse_args()
    ...
    check("root-version",package.get("version")==VERSION,str(package.get("version"))); check("root-description","STEP020D" in package.get("description",""));
    check("acceptance-script",scripts.get("acceptance:step020d")=="python scripts/run_step020d_acceptance.py"); check("live-script",scripts.get("acceptance:step020d:live")=="python scripts/run_step020d_acceptance.py --require-windows-maintenance-live"); check("maintenance-live-script",scripts.get("maintenance-live:step020d")=="node scripts/run-step020d-maintenance-live.mjs")
    ...
    required=[
      "packages/tasks/src/maintenance.ts","packages/task-flows/src/maintenance.ts","packages/state/migrations/021_task_and_flow_maintenance_retention.sql","packages/conversations/src/service.ts",
      "packages/protocol/src/task-operations.ts","packages/protocol/src/task-flow-operations.ts","services/agent-host/src/lifecycle.ts","services/agent-host/src/transport/operation-registry.ts",
      *PRODUCT,"tests/unit/validation-governance-step020d.test.mjs","docs/research/STEP020D_OPENCLAW_TASK_AND_FLOW_MAINTENANCE_AUDIT.md","docs/plans/STEP020D_TASK_AND_TASK_FLOW_RECONCILIATION_LOST_AND_RETENTION_FOUNDATION.md",
      "reference/validation/STEP020C_WINDOWS_BOUND_CONTROLLER_LIVE_ACCEPTANCE.md","reference/validation/STEP020D_LOCAL_SOURCE_PACKAGE_ACCEPTANCE.md",*[f"reference/validation/STEP020D_OR_ISSUE_{number}.md" for number in range(247,259)],
      "scripts/run-step020d-maintenance-live.mjs","scripts/package_step020d.py",
    ]
    ...
    if args.require_windows_maintenance_live:
        result=run("windows-maintenance-live",["node","scripts/run-step020d-maintenance-live.mjs"],700); seconds+=result.elapsed_seconds
        marker=f"{STEP} checks=16/16 state=PASSED version={VERSION} schema={SCHEMA} maintenance=AUDIT_RECONCILE_RETENTION authority=RUN_RUNTIME_SOT lost=RECOVERY_GRACE_RUNTIME_AUTHORITY startup=SAFE_RECONCILE_NO_RETENTION flow=CONTROLLER_OWNED_OUTCOME cancellation=STUCK_REPLAY_FINALIZE retention=PREVIEW_SCHEDULE_NO_PRUNE idempotency=REPEATED_APPLY_STABLE plan_executor=DEFERRED provider=SCRIPTED_LOCAL live_harness={LIVE_HARNESS}"
        check("windows-maintenance-live",result.ok and marker in result.output,result.output[-5000:])
    passed=sum(1 for _,ok,_ in checks if ok); total=len(checks); state="PASSED" if passed==total else "FAILED"; live="PASSED" if args.require_windows_maintenance_live and state=="PASSED" else "PENDING_ENV" if not args.require_windows_maintenance_live else "FAILED"; promotion="READY" if live=="PASSED" else "WINDOWS_MAINTENANCE_LIVE_PENDING" if live=="PENDING_ENV" else "BLOCKED"
    marker=(f"{STEP} checks={passed}/{total} state={state} version={VERSION} schema={SCHEMA} accepted_product_baseline={BASELINE} accepted_checks={BASELINE_CHECKS} maintenance=AUDIT_RECONCILE_RETENTION authority=RUN_RUNTIME_SOT lost=RECOVERY_GRACE_RUNTIME_AUTHORITY startup=SAFE_RECONCILE_NO_RETENTION flow=CONTROLLER_OWNED_OUTCOME cancellation=STUCK_REPLAY_FINALIZE retention=PREVIEW_SCHEDULE_NO_PRUNE idempotency=REPEATED_APPLY_STABLE plan_executor=DEFERRED openclaw_reference=TASK_AND_FLOW_MAINTENANCE_SOURCE_AUDITED external_model=NOT_RUN browser_live=NOT_RUN connector=DEFERRED_NO_REAL_SYSTEM focused_product={counts.get('focused-step020d-product',0)} affected_regression={counts.get('affected-maintenance-regression',0)} governance={counts.get('focused-validation-governance',0)} canonical_files={counts.get('canonical-files',0)} canonical_tests={counts.get('canonical-suite',0)} windows_maintenance_live={live} live_harness={LIVE_HARNESS} promotion={promotion} automated_run_seconds={seconds:.3f}")
    lines=[marker]+[f"OPENRILL_STEP020D_FAILURE check={name}\n{detail}" for name,ok,detail in checks if not ok]; write_acceptance_report(REPORT,"\n".join(lines)+"\n"); print(marker); [print(line) for line in lines[1:]]; return 0 if state=="PASSED" else 1
