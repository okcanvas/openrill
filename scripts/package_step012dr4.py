...
def main() -> int:
    parser = argparse.ArgumentParser(description="Create deterministic OpenRill STEP012DR4 source ZIP")
    ...
    for path in ROOT.rglob("*"):
        ...
        if any(part in EXCLUDED_DIRS for part in relative.parts) or path.suffix in {".pyc", ".pyo"}: continue
        files.append(path)
    ...
    print(f"OPENRILL_STEP012DR4_PACKAGE_PASS files={len(files)} sha256={digest}")
    return 0
