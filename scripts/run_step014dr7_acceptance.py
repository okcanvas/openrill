...
STEP = "STEP014DR7_LIVE_ACCEPTANCE_TRANSPORT_AND_LIFECYCLE_CLOSURE_AUDIT"
...
REPORT = resolve_acceptance_report(ROOT, ".artifacts/acceptance/STEP014DR7_ACCEPTANCE_REPORT.txt")
STAGE_LOG_DIR = REPORT.parent / "STEP014DR7_STAGES"
...
STAGES: list[tuple[str, list[str], int]] = [
    ...
    ("external-model-parallel-live", ["node", "scripts/run-step014dr7-external-model-live.mjs"], 300),
    ("deterministic-nested-control-ui-live", ["node", "scripts/run-step014dr7-deterministic-nested-ui-live.mjs"], 180),
...
def main() -> int:
    ...
    check("current-acceptance-script", scripts.get("acceptance:step014dr7") == "python scripts/run_step014dr7_acceptance.py")
    check("current-package-script", scripts.get("package:step014dr7") == "python scripts/package_step014dr7.py --output ../openrill-step014dr7-live-acceptance-transport-lifecycle-closure-audit-v1.zip")
    ...
    required = [
        ...
        "docs/plans/STEP014DR5_CONTROL_UI_ENTRYPOINT_DISCOVERY_AND_STATIC_ASSET_ROUTE_ALIGNMENT.md", "docs/plans/STEP014DR7_LIVE_ACCEPTANCE_TRANSPORT_AND_LIFECYCLE_CLOSURE_AUDIT.md",
        ...
        "scripts/run-step014dr7-external-model-live.mjs", "scripts/run-step014dr7-deterministic-nested-ui-live.mjs", "scripts/step014dr6-deterministic-nested-fixture.mjs",
    ...
    external_live = read_utf8("scripts/run-step014dr7-external-model-live.mjs")
    nested_ui_live = read_utf8("scripts/run-step014dr7-deterministic-nested-ui-live.mjs")
    ...
    check("dr6-nested-deterministic-ui", "seedDeterministicNestedDelegationFixture" in nested_ui_live and "depth===2" in nested_ui_live and "resolveChromiumExecutable" in nested_ui_live)
    check("dr6-live-separation", 'required("OPENAI_API_KEY")' in external_live and 'required("OPENAI_API_KEY")' not in nested_ui_live)
    check("dr7-live-client-version", all('clientVersion:"0.14.10-step014dr7"' in body and "0.14.9-step014dr7" not in body for body in (external_live, nested_ui_live)))
    check("dr7-retained-nested-fixture-owner", './step014dr6-deterministic-nested-fixture.mjs' in nested_ui_live and './step014dr7-deterministic-nested-fixture.mjs' not in nested_ui_live and (ROOT / "scripts/step014dr6-deterministic-nested-fixture.mjs").is_file())
    ...
    for name, command, timeout in STAGES:
        ...
        if name == "external-model-parallel-live":
            ok = ok and "STEP014DR7_EXTERNAL_MODEL_PARALLEL_PASS" in output
        if name == "deterministic-nested-control-ui-live":
            ok = ok and "STEP014DR7_DETERMINISTIC_NESTED_UI_PASS" in output and "chromium_orphan=0" in output
