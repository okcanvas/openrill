...
- 작업은 하나의 STEP branch 또는 패키지 기준선으로 제한한다.
- 커밋 메시지에 STEP 번호를 포함한다.
- 새로운 기능은 먼저 `docs/plans`의 수용 기준을 갱신한다.
- 공개 계약 변경은 ADR과 protocol/schema test를 동반한다.
- OpenClaw를 읽고 구현한 경우 해당 STEP의 Reference Evidence 표에 경로·symbol을 추가하되 코드를 복사하지 않는다.
- 외부 의존성 추가 전 license, maintenance, platform support, transitive native dependency를 기록한다.
