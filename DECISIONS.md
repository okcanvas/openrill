...
1. OpenClaw actual source is the reference answer key; OpenRill adopts only code-proven contracts compatible with its architecture.
2. Goal/Plan, Task and Task Flow are separate durable concepts.
3. STEP020B Task Flow is controller-owned registry state, not a general scheduler and not an autonomous Plan executor.
4. A Flow may link many Tasks; a Task belongs to at most one Flow.
5. Non-replay mutations require expected revision; terminal status is monotone.
6. Flow cancellation records intent and delegates through each child Task to its owning Run.
7. Workspace authorization remains in Product service boundaries; migration 019 does not add a stronger `workspace_registrations` precondition.
8. Official accepted baseline remains STEP020A until the STEP020B Windows live harness passes.
