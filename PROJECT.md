...
OpenRill now has three intentionally separate durable layers:

- Goal/Plan: Conversation-owned intent, revisioned ordered proposal and checkpointed progress.
- Task: one-to-one Run-linked execution activity ledger across Conversation, Delegation and Automation.
- Task Flow: controller-owned orchestration state linking several Tasks with revision conflict detection and wait/block/resume/cancel lifecycle.

Task Flow does not schedule itself. Actual execution remains owned by Conversation, Delegation or Automation runtime and Run lifecycle.

## Accepted baseline

`STEP020A_DURABLE_BACKGROUND_TASK_LEDGER_AND_RUNTIME_LIFECYCLE_FOUNDATION`, checks 40/40, schema 18, SHA-256 `67ac1fa4a5067ff3070f0a990bfdfd262a6d956961ebd221432cdacf567c9a7f`.

## Reference

OpenClaw `2026.7.2` from `openclaw-main.zip` is the answer key. STEP020B audit is recorded in `docs/research/STEP020B_OPENCLAW_TASK_FLOW_REFERENCE_AUDIT.md`.

## Product limits

No autonomous Plan-to-Task executor, notifications, flow maintenance, LOST sweeper, retention, distributed workers, external model, Browser LIVE or real Connector is claimed by STEP020B.
