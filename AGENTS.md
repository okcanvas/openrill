...
1. 추측으로 현재 구현 상태를 선언하지 않는다.
2. 구현 전 해당 STEP의 `Reference Evidence`와 실제 대상 코드를 읽는다.
3. 리뷰 요청에서 사용자가 수정 지시를 하지 않으면 파일을 변경하지 않는다.
4. OpenClaw 코드, 타입, 테스트, 문구를 복사하지 않는다.
5. OpenClaw compatibility를 목표로 하는 API를 추가하지 않는다.
6. Windows launcher와 경로는 실제 Windows 수용 전 통과로 기록하지 않는다.
7. 모든 상태 변경은 재시도·중단·재시작 시 의미를 정의한다.
8. 모든 위험 Tool은 policy 결과와 approval binding을 기록한다.
9. 각 STEP은 source audit → plan → implementation → deterministic gate → Windows live gate 순서다.
10. ZIP 패키징 시 SHA-256과 현재 STEP을 `HANDOFF.md`에 기록한다.
11. 실제 실패나 반복 가능 결함은 `docs/governance/ENGINEERING_ISSUE_REGISTRY.md`와 상세 실패 문서에 기록하고 자동 재발 방지 gate를 추가한다.

## 코드 작성 원칙

- TypeScript strict mode
- feature facade는 작게 유지하고 내부 모듈을 지연 import할 수 있다.
- wire schema에서 타입을 생성하고, 타입만으로 입력을 신뢰하지 않는다.
- SQLite schema 변경은 migration과 fresh-db/idempotent 테스트를 동반한다.
- event는 append-only를 기본으로 하고 projection은 재생성 가능해야 한다.
- Tool Call id와 Approval id는 재사용하지 않는다.
- Tool 실행 후 결과 저장이 실패하면 성공으로 사용자에게 표시하지 않는다.
- config source와 runtime snapshot을 구분한다.
- Secret 값은 config/log/event에 저장하지 않는다.

## 문서 갱신

구현 STEP 완료 시 최소 갱신 대상:

- `PLANS.md`
- `ROADMAP.md`
- `HANDOFF.md`
- `VALIDATION.md`
- 해당 `docs/plans/STEPxxx_*.md`
- 변경된 계약 문서
- `docs/governance/ENGINEERING_ISSUE_REGISTRY.md` 및 재발 방지 gate(새 이슈가 발생한 경우)
