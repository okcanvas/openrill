...
Required source/package pipeline:

1. source version, lock and current-root module-link alignment;
2. package manifest verification;
3. clean workspace build;
4. STEP020B focused Product tests;
5. affected STEP020A/STEP019B/Goal/Delegation/Automation/Conversation/State regression;
6. cumulative validation governance including OR-ISSUE-237, OR-ISSUE-238 and OR-ISSUE-239;
7. canonical unit suite;
8. architecture and export boundaries;
9. final package manifest verification.
...
Expected harness identity: `STEP020B_H1_TASK_FLOW_PROTOCOL_RESTART_REVISION_AND_CANCELLATION`.

No source/package result may claim `windows_task_flow_live=PASSED`. External model, Browser LIVE and real Connector are intentionally outside this acceptance.

## Failure continuity

- `OR-ISSUE-237`: migration 019 initially imposed an unrelated `workspace_registrations` FK; removed before acceptance.
- `OR-ISSUE-238`: the first canonical run found `OR-ISSUE-213` missing from current `HANDOFF.md` and `VALIDATION.md`; both tokens are now retained and governed.
- `OR-ISSUE-239`: the second canonical run found retained `OR-ISSUE-214`/H2 privacy identity missing from the same assets; it is restored and all canonical handoff readers are preflighted.

The retained `OR-ISSUE-213` gate remains authoritative for race-safe handling of a child process whose
close event was observed before listener registration.
The retained `OR-ISSUE-214` gate keeps authorized history visibility separate from secret leakage and prompt echo outside explicit history.

## Document-inclusive source/package acceptance

```text
checks=36/36 state=PASSED
focused_product=6/6
affected_regression=73/73
governance=127/127
canonical=130 files / 706/706 / skipped 0
